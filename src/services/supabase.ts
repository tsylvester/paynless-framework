...
export const getUser = async () => {
  try {
    ...
    if (storedSession) {
      try {
        ...
        if (parsedSession && parsedSession.user) {
          ...
          if (!networkMonitor.isOnline()) {
            eventEmitter.emit('user-loaded', { user: true });
    ...
    if (!networkMonitor.isOnline()) {
      ...
      eventEmitter.emit('user-loaded', { user: false });
      return null;
    }
...
export const getSession = async () => {
  try {
    ...
    let validSession: boolean = false;
    if (data.session) {
      validSession = true;
      logger.debug('Found existing session in getSession');
    }
    ...
    if (error) {
      ...
      if (errorType === AuthErrorType.NOT_AUTHENTICATED ||
        errorType === AuthErrorType.EXPIRED_SESSION) {
        eventEmitter.emit('session-checked', { session: false });
        return null;
      }
    ...
    if (data.session && networkMonitor.isOnline()) {
      try {
        ...
        if (refreshError) {
          ...
          eventEmitter.emit('session-checked', { session: true });
          return data.session;
        }
        eventEmitter.emit('session-checked', { session: true });
    ...
    eventEmitter.emit('session-checked', { session: validSession });
    return data.session;
  } catch (error) {
