...
CREATE POLICY "Users can update status of received messages"
  ...
  WITH CHECK (auth.uid() = recipient_id AND (status <> OLD.status));
...
-- Create triggers to update post counts - Fixed to properly handle OLD/NEW tables
...
-- Fixed like count function and triggers
