...
export function isAssembleAiResponseParams(value: unknown): value is AssembleAiResponseParams {
  ...
  if (value.preflightInputTokens !== undefined && (typeof value.preflightInputTokens !== "number" || !Number.isFinite(value.preflightInputTokens) || value.preflightInputTokens < 0)) return false;
...
export function isAssembleAiResponseErrorReturn(value: unknown): value is AssembleAiResponseErrorReturn {
  if (!isRecord(value)) return false;
  if (!isAssembleAiResponseTokenCountError(value.error) && !isAssembleAiResponseMissingPreflightError(value.error)) return false;
...
export function isAssembleAiResponseMissingPreflightError(value: unknown): value is AssembleAiResponseMissingPreflightError {
  return value instanceof AssembleAiResponseMissingPreflightError;
}
