...
export async function submitStageDocumentFeedback(
  ...
  if (resolvedSourceContributionId === null) {
    logger.warn('sourceContributionId is required for feedback placement.', {
      sessionId,
      stageSlug,
      iterationNumber,
      documentKey,
  ...
  if (originalStoragePath === undefined || originalBaseName === undefined) {
    logger.warn('Original document placement fields were not resolved.', {
      sourceContributionId: resolvedSourceContributionId,
    });
    return {
      error: { message: 'Original document not found for feedback placement.' },
  ...
  const feedbackBytes: Uint8Array = new TextEncoder().encode(feedbackContent);
  const sizeBytes: number = feedbackBytes.byteLength;
  ...
  if (fileError || !fileRecord) {
    const fileErrorMessage: string = fileError ? fileError.message : 'Unknown file manager error';
    const detailsFromFileError: string | undefined =
      fileError && 'details' in fileError && typeof fileError.details === 'string'
        ? fileError.details
        : undefined;
    logger.error(
    ...
    return {
      error: { message: fileErrorMessage, details: detailsFromFileError },
    };
  }

  if (!('feedback_type' in fileRecord)) {
    logger.error('File record from fileManager is not a dialectic_feedback row.', {
      fileRecord,
    });
