 ...
 */
export async function syncGoogleModels(supabaseClient: SupabaseClient, apiKey: string): Promise<SyncResult> {
  let insertedCount = 0;
  let updatedCount = 0;
  ...
  try {
    ...
    const apiModels = await googleAdapter.listModels(apiKey); 
    ...
    if (modelsToInsert.length > 0) {
      ...
      try {
          ...
          if (error) {
               console.error(`Insert resolved with error object for ${PROVIDER_NAME}:`, error);
               throw new Error(`Insert failed for ${PROVIDER_NAME}: ${error.message}`);
          }
          insertedCount = modelsToInsert.length;
      } catch (err) { // Catch the rejected promise from await
          console.error(`Insert await threw error for ${PROVIDER_NAME}:`, err);
    ...
    if (modelsToUpdate.length > 0) {
      ...
      try {
          for (const update of modelsToUpdate) {
              ...
              if (error) {
                  console.error(`Update resolved with error object for model ID ${update.id} (${PROVIDER_NAME}):`, error);
                  // Throw specific error if promise resolves with error property
                  throw new Error(`Update failed for model ID ${update.id} (${PROVIDER_NAME}): ${error.message}`);
              }
          }
          updatedCount = modelsToUpdate.length;
      } catch (err) { // Catch rejection from any update await
           console.error(`Update await threw error for ${PROVIDER_NAME}:`, err);
    ...
    if (modelsToDeactivate.length > 0) {
      console.log(`Deactivating ${modelsToDeactivate.length} ${PROVIDER_NAME} models...`);
       try {
            ...
            if (error) {
                console.error(`Deactivation resolved with error object for ${PROVIDER_NAME}:`, error);
                throw new Error(`Deactivation failed for ${PROVIDER_NAME}: ${error.message}`);
            }
            deactivatedCount = modelsToDeactivate.length;
        } catch (err) { // Catch rejection from deactivation await
             console.error(`Deactivation await threw error for ${PROVIDER_NAME}:`, err);
