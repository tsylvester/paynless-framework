...
export function isHeaderContextDocuments(value: unknown): value is ContextForDocument[] {
    ...
    // HeaderContext.context_for_documents must have valid FileType document_key values
    // (not just any string, since header context is generated by PLAN jobs for known documents)
    return value.every(entry => {
        if (!isContextForDocument(entry)) {
            return false;
        }
        // Additional validation: document_key must be a valid FileType for header context
        return typeof entry.document_key === 'string' && isFileType(entry.document_key);
    });
}
...
export function isHeaderContext(value: unknown): value is HeaderContext {
    if (!isRecord(value)) return false;
    ...
    // Reject objects with files_to_generate property (not part of HeaderContext interface)
...
export function isContentToInclude(value: unknown): value is ContentToInclude {
    // Must be an object, not an array at top level
    if (!isRecord(value)) {
        return false;
    }
    ...
    for (const key in value) {
        ...
        if (Array.isArray(itemValue)) {
            ...
            // Check if it's string[] (all elements are strings)
            const allStrings = itemValue.every(element => typeof element === 'string');
            if (allStrings) {
                continue;
            }
            
            // Check if it's ContentToInclude[] (all elements pass isContentToInclude recursively)
            const allContentToInclude = itemValue.every(element => isContentToInclude(element));
            if (allContentToInclude) {
                continue;
            }
            
            // Mixed array or invalid types
            return false;
        }
        
        // Handle nested objects (recursive ContentToInclude)
        if (isRecord(itemValue)) {
            if (!isContentToInclude(itemValue)) {
                return false;
            }
            continue;
        }
...
export function isContextForDocument(value: unknown): value is ContextForDocument {
    ...
    return (
        ...
        // Note: TypeScript interface says FileType, but recipes can introduce dynamic document keys.
        'document_key' in value && typeof value.document_key === 'string' && value.document_key.length > 0 &&
        'content_to_include' in value && isContentToInclude(value.content_to_include)
    );
}
...
export function isDialecticPlanJobPayload(payload: unknown): payload is DialecticPlanJobPayload {
    ...
    if ('continueUntilComplete' in payload && typeof payload.continueUntilComplete !== 'boolean') return false;
    if ('maxRetries' in payload && typeof payload.maxRetries !== 'number') return false;
    if ('continuation_count' in payload && typeof payload.continuation_count !== 'number') return false;
    if ('target_contribution_id' in payload && typeof payload.target_contribution_id !== 'string') return false;
    if ('model_slug' in payload && typeof payload.model_slug !== 'string') return false;
...
export function isDialecticRenderJobPayload(payload: unknown): payload is DialecticRenderJobPayload {
    if (!isRecord(payload)) {
        throw new Error('Payload must be a non-null object.');
    }
...
export function isPlanJobInsert(item: unknown): item is PlanJobInsert {
    ...
    if ('job_type' in payload) return false;
