...
export enum FileType {
  ...
  CompressedContext = 'compressed_context', // The compressed representation of a context victim, persisted as a machine-only _work artifact.
  CompressedContextRawJson = 'compressed_context_raw_json', // The raw JSON response from a compression model call, persisted as a machine-only _work artifact.
  ...
  CompressionPrompt = 'compression_prompt', // The prompt sent by a COMPRESS job, persisted as a machine-only _work artifact.
...
/**
 * The source of a compression victim; determines which identity field is required.
 */
export type CompressionSourceType = 'contribution' | 'resource' | 'feedback' | 'history';

/**
 * The compression mode for a COMPRESS job: JSON mode preserves source structure,
 * text mode is freeform compression.
 */
export type CompressionMode = 'json' | 'text';
...
export interface PathContext {
  ...
  chunkTotal?: number; // map-reduce chunk total; both-or-neither; must be >= chunkIndex
  documentKey?: FileType; // e.g., 'executive_summary', 'technical_design'
...
export type ResourceFileTypes = 
  ...
  | FileType.CompressedContext
  | FileType.CompressedContextRawJson
  | FileType.CompressionPrompt
