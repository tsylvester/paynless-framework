...
export async function handleCheckoutSessionCompleted(
  ...
  if (session.mode === 'subscription') {
    ...
    try {
      ...
      // This update should ideally happen, but the primary goal here is to signal a server-side issue.
      try {
          await context.updatePaymentTransaction(internalPaymentId, { status: 'FAILED' }, event.id);
      } catch (updateError) {
          context.logger.error(`[handleCheckoutSessionCompleted] Failed to update payment transaction ${internalPaymentId} to FAILED after subscription retrieve error.`, { nestedError: updateError });
          // Log this failure but still re-throw the original retrieveError
      }
      // Re-throw the original error so the main webhook router can handle it as a 500
      throw retrieveError; 
    }
