...
import type { AiProviderAdapter, ChatApiRequest, AiModelExtendedConfig, AdapterResponsePayload, ProviderModelInfo } from '../types.ts';
 ...
 * @param providerModelConfig - A valid, provider-specific config to test with.
 */
export async function testAdapterContract(
    t: Deno.TestContext,
    AdapterClass: AiProviderAdapter,
    mockApi: MockApi,
    providerModelConfig: AiModelExtendedConfig,
) {
    const modelId = providerModelConfig.api_identifier;

    await t.step(`[Contract] ${AdapterClass.name} - Instantiation`, () => {
        const adapter = new AdapterClass(MOCK_API_KEY, MOCK_LOGGER, providerModelConfig);
    ...
    await t.step(`[Contract] ${AdapterClass.name} - sendMessage: Throws on excessive prompt tokens`, async () => {
        ...
        const smallModelConfig: AiModelExtendedConfig = {
            ...providerModelConfig,
            provider_max_input_tokens: 5, 
        };
        const adapter = new AdapterClass(MOCK_API_KEY, MOCK_LOGGER, smallModelConfig);
