...
export const handleCorsPreflightRequest = (req: Request): Response | null => {
	...
	if (req.method === "OPTIONS") {
		...
		// Debug: Always log if Access-Control-Allow-Origin is present
		if (corsHeaders["Access-Control-Allow-Origin"]) {
			console.log(
				`[cors-headers] ✅ CORS headers include Access-Control-Allow-Origin: ${corsHeaders["Access-Control-Allow-Origin"]}`,
			);
		} else {
			console.log(
				`[cors-headers] ❌ CORS headers DO NOT include Access-Control-Allow-Origin`,
			);
		}
