...
export async function runSpecificConfigsTests(
  ...
  await t.step("[Specific Config] Chat with a non-existent system_prompt_id", async () => {
    ...
    // Expecting a 404 or a specific error message if the prompt ID is not found
    // Or, if it falls back gracefully, it should be a 200 OK.
    // For now, let's assume it should fall back and respond normally.
    // If it's supposed to error, this assertion will need to change.
  ...
  await t.step("[Context Handling] Chat continuation with existing chatId", async () => {
    ...
    assertEquals(firstResponseJson.assistantMessage.content, `Echo from Dummy: ${firstMessageContent}`);
    ...
    assertEquals(secondResponseJson.assistantMessage.content, `Echo from Dummy: ${secondMessageContent}`, "Dummy provider did not echo second message correctly.");
    ...
    assertEquals(messages[1].content, `Echo from Dummy: ${firstMessageContent}`);
    assertEquals(messages[1].role, "assistant");
    assertEquals(messages[2].content as string, secondMessageContent);
    assertEquals(messages[2].role, "user");
    assertEquals(messages[3].content, `Echo from Dummy: ${secondMessageContent}`);
    assertEquals(messages[3].role, "assistant");

    // Check wallet balance - should still be unchanged for dummy provider
    const { data: wallet, error: walletErr } = await scAdminClient
        .from('token_wallets')
        .select('balance')
        .eq('user_id', testUserId) 
  ...
  await t.step("[Context Handling] Chat continuation with selected messages", async () => {
    ...
    assertEquals(secondResp.status, 200, "Selection Test: Second message failed.");
    const secondRespJson = JSON.parse(await secondResp.text()) as ChatHandlerSuccessResponse;
    ...
    assertEquals(thirdRespJson.assistantMessage.content, `Echo from Dummy: ${thirdMsgContent}`, "Dummy provider should echo the third message.");
  ...
  // All tests from the checklist for specific_configs should now be implemented.
} 