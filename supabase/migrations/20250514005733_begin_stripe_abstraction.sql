...
ADD COLUMN tokens_to_award NUMERIC(19,0),
...
-- Similarly for tokens_to_award, if it should always be NOT NULL for certain plan_types:
-- (This might be better as a CHECK constraint or handled by application logic)
-- Example: ALTER TABLE public.subscription_plans ADD CONSTRAINT check_tokens_for_one_time CHECK (plan_type <> 'one_time_purchase' OR tokens_to_award IS NOT NULL);
