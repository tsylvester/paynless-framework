-- Migration: Correct the handle_new_invite_notification trigger function
...
BEGIN
  ...
  IF invited_user_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      COALESCE(inviter_name, 'Someone') || ' has invited you to join ' || COALESCE(organization_name, 'an organization') || ' as a ' || NEW.role_to_assign || '.', -- Message with fallbacks
      jsonb_build_object(
        ...
        'inviter_name', inviter_name, -- Store the determined name/email
