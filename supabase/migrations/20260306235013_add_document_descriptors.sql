...
-- Add display_name and description to document entries in outputs_required JSONB (idempotent).
-- Only markdown document entries across dialectic_recipe_template_steps and dialectic_stage_recipe_steps.
WITH doc_meta(document_key, display_name, description) AS (
  VALUES
  ...
  SELECT
    ...
    jsonb_set(
      ...
      (
        SELECT jsonb_agg(
          CASE
            WHEN elem->>'file_type' = 'markdown' THEN
              elem || COALESCE(
                (
                  SELECT jsonb_build_object('display_name', m.display_name, 'description', m.description)
                  FROM doc_meta m
                  WHERE m.document_key = elem->>'document_key'
                ),
                '{}'::jsonb
              )
            ELSE elem
        ...
        FROM jsonb_array_elements(s.outputs_required->'documents') WITH ORDINALITY AS t(elem, ord)
      )
    ) AS new_outputs_required
  FROM public.dialectic_recipe_template_steps s
  WHERE s.outputs_required ? 'documents'
)
UPDATE public.dialectic_recipe_template_steps s
SET outputs_required = e.new_outputs_required
FROM enriched_template e
WHERE s.id = e.id;
