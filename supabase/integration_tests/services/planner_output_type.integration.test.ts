  ...
  describe("assembleTurnPrompt cross-document coordination (step 33)", () => {
    it("should read files_to_generate from recipe step and use context_for_documents for alignment (step 33.b.i, 33.b.ii)", async () => {
      ...
      const headerContextContent: HeaderContext = {
        system_materials: {
          agent_internal_summary: "Test agent internal summary for cross-document coordination",
...
This is a test template for product requirements document generation.`;
    ...
    it("should verify complete PLAN → EXECUTE flow with header_context generation and consumption (step 33.b.iii)", async () => {
      ...
      const headerContextContent: HeaderContext = {
        system_materials: {
          agent_internal_summary: "Test agent internal summary for PLAN → EXECUTE flow",
          input_artifacts_summary: "Test input artifacts summary",
          stage_rationale: "Test stage rationale",
        },
        header_context_artifact: {
    ...
    it("should verify structure matching between files_to_generate and context_for_documents (step 33.b.iv)", async () => {
      ...
      const headerContextContent: HeaderContext = {
        system_materials: {
          agent_internal_summary: "Test agent internal summary",
          input_artifacts_summary: "Test input artifacts summary",
          stage_rationale: "Test stage rationale",
        },
        header_context_artifact: {
  ...
  describe("Complete end-to-end flow with correct file paths and step tracking (step 43)", () => {
    it("should verify EXECUTE jobs have document_key, file paths use documentKey, notifications use document_key, and completed steps are not re-planned (step 43.b)", async () => {
      ...
      // Verify at least one EXECUTE job was created
      assert(childJobs.length > 0, "testPlannerWithRecipeStep should create at least one EXECUTE job");
      
      // Find an EXECUTE job (not a PLAN job)
      const executeJob = childJobs.find(job => {
        if (!isDialecticExecuteJobPayload(job.payload)) return false;
        return job.payload.document_key === "business_case";
      });

      if (!executeJob) {
        throw new Error("No EXECUTE job found with document_key 'business_case'");
      }

      if (!isDialecticExecuteJobPayload(executeJob.payload)) {
        throw new Error("Execute job payload is not a valid DialecticExecuteJobPayload");
      }
      const executePayload = executeJob.payload;
      
      // Verify document_key is set correctly (43.b.i)
      assertExists(executePayload.document_key, "EXECUTE job payload must have document_key");
      assertEquals(
        executePayload.document_key,
        "business_case",
      ...
      const originalUploadAndRegisterFile = workerDeps.fileManager.uploadAndRegisterFile.bind(workerDeps.fileManager);
      workerDeps.fileManager.uploadAndRegisterFile = async (context: ModelContributionUploadContext) => {
      ...
      if (providerError || !providerData) {
        throw new Error(`Failed to fetch provider data: ${providerError?.message || "Provider not found"}`);
      }

      // Build prompt construction payload (minimal for test)
      const { assembleTurnPrompt } = await import("../../functions/_shared/prompt-assembler/assembleTurnPrompt.ts");
      
      // Fetch project, session, and stage for assembleTurnPrompt
      const { data: projectRecord, error: projectError } = await adminClient
        .from("dialectic_projects")
        .select("*, dialectic_domains(name)")
        .eq("id", testProject.id)
      ...
      // Build stage context with recipe step
      if (!recipeStep.outputs_required || !Array.isArray(recipeStep.outputs_required) || recipeStep.outputs_required.length === 0) {
        throw new Error("Recipe step must have outputs_required array with at least one element");
      }
      const firstOutputRule = recipeStep.outputs_required[0];
      if (!isOutputRule(firstOutputRule)) {
        throw new Error("Recipe step outputs_required[0] must be a valid OutputRule");
      }
      ...
      const dialecticRecipeStep: DialecticStageRecipeStep = {
        ...
        outputs_required: firstOutputRule,
        step_description: "",
      };
      ...
      // Assemble prompt for executeModelCallAndSave
      ...
      const executeParams: ExecuteModelCallAndSaveParams = {
        dbClient: adminClient,
        deps: workerDeps,
        ...
        promptConstructionPayload: {
          systemInstruction: assembledPrompt.promptContent,
          conversationHistory: [],
          resourceDocuments: [],
          currentUserPrompt: "",
          source_prompt_resource_id: assembledPrompt.source_prompt_resource_id,
        },
      ...
      await executeModelCallAndSave(executeParams);

      // Verify file path uses documentKey (43.b.ii)
      assertExists(capturedPathContext, "fileManager.uploadAndRegisterFile should have been called");
      if (!capturedPathContext) {
        throw new Error("capturedPathContext is null - fileManager.uploadAndRegisterFile was not called");
      }
      const pathContext: ModelContributionUploadContext["pathContext"] = capturedPathContext;
      assertExists(
        pathContext.documentKey,
        "pathContext should have documentKey set for document file types"
      );
      assertEquals(
        pathContext.documentKey,
        "business_case",
        `pathContext.documentKey should be 'business_case', got '${pathContext.documentKey}'`
      );
      ...
      // Create a new PLAN job using the actual recipe to test step tracking
      const generateResult = await generateContributions(
        adminClient,
        {
          sessionId: testSession.id,
      ...
      assert(generateResult.success, `generateContributions failed: ${generateResult.error?.message}`);
      if (!generateResult.data || !generateResult.data.job_ids || generateResult.data.job_ids.length === 0) {
        throw new Error("generateContributions did not create a PLAN job");
      }

      const freshPlanJobId = generateResult.data.job_ids[0];
      
      // Process the PLAN job ONCE to create the first step's child jobs
      const { data: freshPlanJob, error: freshPlanJobError } = await adminClient
        .from('dialectic_generation_jobs')
        .select('*')
        .eq('id', freshPlanJobId)
        .single();

      if (freshPlanJobError || !freshPlanJob) {
        throw new Error(`Failed to fetch fresh PLAN job: ${freshPlanJobError?.message || "Job not found"}`);
      }

      if (!isDialecticPlanJobPayload(freshPlanJob.payload)) {
        throw new Error("Fresh PLAN job payload is not a valid DialecticPlanJobPayload");
      }

      // Process the PLAN job once to create child jobs for build-stage-header
      await handleJob(
        adminClient,
        freshPlanJob,
        workerDeps,
      ...
      // Fetch the child job created for build-stage-header
      const { data: headerChildJobs, error: headerChildJobsError } = await adminClient
        .from('dialectic_generation_jobs')
        .select('*')
        .eq('parent_job_id', freshPlanJobId)
        .eq('status', 'pending');

      if (headerChildJobsError) {
        throw new Error(`Failed to fetch header child jobs: ${headerChildJobsError.message}`);
      }

      if (!headerChildJobs || headerChildJobs.length === 0) {
        throw new Error("No child jobs created for build-stage-header step");
      }

      // Complete one of the child jobs (simulating it finishing)
      const headerChildJob = headerChildJobs[0];
      
      // Verify the child job type - build-stage-header creates EXECUTE child jobs (not PLAN)
      assertEquals(
        headerChildJob.job_type,
        "EXECUTE",
        `Header child job should be EXECUTE type (build-stage-header PLAN step creates EXECUTE children), got '${headerChildJob.job_type}'`
      );
      
      // Verify the child job has planner_metadata.recipe_step_id set correctly
      if (!isDialecticExecuteJobPayload(headerChildJob.payload)) {
        throw new Error(`Header child job must have a valid EXECUTE payload. Got job_type: ${headerChildJob.job_type}, payload type check failed.`);
      }
      const headerPayload = headerChildJob.payload;
      if (!headerPayload.planner_metadata || !headerPayload.planner_metadata.recipe_step_id) {
        throw new Error(`Header child job must have planner_metadata.recipe_step_id. Got: ${JSON.stringify(headerPayload.planner_metadata)}`);
      }
      const headerRecipeStepId = headerPayload.planner_metadata.recipe_step_id;
      assertEquals(
        headerRecipeStepId,
        "f9053ca2-5a5c-4794-8239-ee69834a5d98",
        `Header child job recipe_step_id should be 'f9053ca2-5a5c-4794-8239-ee69834a5d98' (build-stage-header), got '${headerRecipeStepId}'`
      );
      
      const { error: completeHeaderJobError } = await adminClient
        .from("dialectic_generation_jobs")
        .update({
          status: "completed",
          completed_at: new Date().toISOString(),
        })
        .eq("id", headerChildJob.id);

      if (completeHeaderJobError) {
        throw new Error(`Failed to complete header child job: ${completeHeaderJobError.message}`);
      }
      
      // Verify the completed child job can be fetched with the correct status
      const { data: completedChildJob, error: fetchCompletedError } = await adminClient
        .from("dialectic_generation_jobs")
      ...
      if (fetchCompletedError || !completedChildJob) {
        throw new Error(`Failed to verify completed child job: ${fetchCompletedError?.message || "Job not found"}`);
      }
      
      // Verify the completed child job still has planner_metadata.recipe_step_id
      if (!isDialecticExecuteJobPayload(completedChildJob.payload)) {
        throw new Error(`Completed child job must have a valid EXECUTE payload. Payload type check failed.`);
      }
      const completedPayload = completedChildJob.payload;
      if (!completedPayload.planner_metadata || !completedPayload.planner_metadata.recipe_step_id) {
        throw new Error(`Completed child job must have planner_metadata.recipe_step_id. Got: ${JSON.stringify(completedPayload.planner_metadata)}`);
      }

      // Set parent to pending_next_step (simulating database trigger)
      ...
      if (updateParentError) {
        throw new Error(`Failed to update parent to pending_next_step: ${updateParentError.message}`);
      }

      // Fetch updated parent job
      const { data: updatedParentJob, error: updatedParentError } = await adminClient
        .from("dialectic_generation_jobs")
        .select("*")
        .eq("id", freshPlanJobId)
        .single();

      if (updatedParentError || !updatedParentJob) {
        throw new Error(`Failed to fetch updated parent job: ${updatedParentError?.message}`);
      }

      if (!isDialecticPlanJobPayload(updatedParentJob.payload)) {
        throw new Error("Updated parent job payload is not a valid DialecticPlanJobPayload");
      }

      // Create properly typed parent job for processComplexJob
      const typedParentJob: DialecticJobRow & { payload: DialecticPlanJobPayload } = {
        ...updatedParentJob,
        payload: updatedParentJob.payload,
      };

      // Verify that processComplexJob can find the completed child job
      // This is what processComplexJob does internally to track completed steps
      const { data: completedChildrenForVerification, error: completedChildrenError } = await adminClient
        .from('dialectic_generation_jobs')
        .select('id, payload')
        .eq('parent_job_id', freshPlanJobId)
        .eq('status', 'completed');

      if (completedChildrenError) {
        throw new Error(`Failed to fetch completed children for verification: ${completedChildrenError.message}`);
      }

      assert(
        (completedChildrenForVerification || []).length > 0,
        `processComplexJob should find at least one completed child job. Found: ${(completedChildrenForVerification || []).length}`
      );

      // Verify the completed child job has the correct recipe_step_id
      const completedChild = (completedChildrenForVerification || []).find(c => c.id === headerChildJob.id);
      assertExists(completedChild, "Completed child job should be found by processComplexJob query");
      
      if (!isDialecticExecuteJobPayload(completedChild.payload)) {
        throw new Error(`Completed child job payload must be a valid EXECUTE payload. Payload type check failed.`);
      }
      const verifiedPayload = completedChild.payload;
      assertExists(
        verifiedPayload.planner_metadata?.recipe_step_id,
        `Completed child job must have planner_metadata.recipe_step_id. Got: ${JSON.stringify(verifiedPayload.planner_metadata)}`
      );
      assertEquals(
        verifiedPayload.planner_metadata.recipe_step_id,
        "f9053ca2-5a5c-4794-8239-ee69834a5d98",
        `Completed child job recipe_step_id should match build-stage-header step ID`
      );

      // Count child jobs before calling processComplexJob
      const { data: childJobsBefore, error: childJobsBeforeError } = await adminClient
        .from("dialectic_generation_jobs")
        .select("id")
        .eq("parent_job_id", freshPlanJobId);

      if (childJobsBeforeError) {
        throw new Error(`Failed to fetch child jobs before: ${childJobsBeforeError.message}`);
      }

      const childJobCountBefore = (childJobsBefore || []).length;

      // Call processComplexJob - it should NOT re-plan build-stage-header since it's already completed
      // NOTE: This will trigger database triggers which may call handleJob again, but processComplexJob
      // should see the completed child job and not re-plan build-stage-header
      ...
      // Count child jobs after calling processComplexJob
      const { data: childJobsAfter, error: childJobsAfterError } = await adminClient
        .from("dialectic_generation_jobs")
        .select("id")
        .eq("parent_job_id", freshPlanJobId);

      if (childJobsAfterError) {
        throw new Error(`Failed to fetch child jobs after: ${childJobsAfterError.message}`);
      }

      const childJobCountAfter = (childJobsAfter || []).length;

      // Verify processComplexJob did NOT create duplicate jobs for build-stage-header (43.b.iv)
      // It should only create jobs for the NEXT steps (generate-business-case, etc.)
      // Since build-stage-header is already completed, no new jobs should be created for it
      const { data: duplicateJobs, error: duplicateJobsError } = await adminClient
        .from("dialectic_generation_jobs")
        .select("id, payload")
        .eq("parent_job_id", freshPlanJobId)
        .eq("status", "pending");

      if (duplicateJobsError) {
        throw new Error(`Failed to fetch duplicate jobs: ${duplicateJobsError.message}`);
      }

      // Verify that no duplicate jobs exist for build-stage-header (recipe_step_id: f9053ca2-5a5c-4794-8239-ee69834a5d98)
      const buildStageHeaderStepId = "f9053ca2-5a5c-4794-8239-ee69834a5d98"; // This is the build-stage-header step ID from the recipe
      const duplicateJobsForBuildStageHeader = (duplicateJobs || []).filter((job) => {
        if (job.payload && typeof job.payload === "object" && "planner_metadata" in job.payload) {
          const plannerMetadata = job.payload.planner_metadata;
          if (plannerMetadata && typeof plannerMetadata === "object" && "recipe_step_id" in plannerMetadata) {
            return plannerMetadata.recipe_step_id === buildStageHeaderStepId;
          }
        }
        return false;
      });
      ...
      assertEquals(
        duplicateJobsForBuildStageHeader.length,
        0,
        `processComplexJob should NOT re-plan already-completed step 'build-stage-header'. Found ${duplicateJobsForBuildStageHeader.length} duplicate jobs for recipe_step_id '${buildStageHeaderStepId}'.`
      );

      // Verify that processComplexJob only planned remaining steps (not the completed one)
      // The thesis recipe has 5 steps, so after build-stage-header completes, the next 4 steps should be planned
      // But we only verify that build-stage-header was NOT re-planned
      assert(
        childJobCountAfter >= childJobCountBefore,
        `processComplexJob should plan remaining steps. Child job count should not decrease. Before: ${childJobCountBefore}, After: ${childJobCountAfter}`
      );
    });
  });
});
