...
- Strip getServerSideProps
  - **Status:** [ ]
  - **Coverage notes:** Essential gating phase. All subsequent development strictly depends on this passing to avoid massive sunk costs.
...
- Migrate core logic to packages/core
  - **Status:** [ ]
  - **Coverage notes:** Establishes the foundational directory structure (`apps/web`, `apps/desktop`, `packages/core`).
  - **Iteration delta:** Initial baseline
...
- Refactor UI logic away from hardcoded window.* calls
  - **Status:** [ ]
  - **Coverage notes:** Isolates environment-specific logic, critical for achieving 85% codebase reuse.
  - **Iteration delta:** Initial baseline
...
- Create 'process_document' Rust invoke handler
  - **Status:** [ ]
  - **Coverage notes:** Rust/Tokio chosen over single-threaded JS strictly to guarantee 60fps UI performance during heavy workloads.
  - **Iteration delta:** Initial baseline
...
- Define OfflineSyncQueue schema
  - **Status:** [ ]
  - **Coverage notes:** Critical for operating smoothly across inconsistent network states in remote enterprise usage.
  - **Iteration delta:** Initial baseline
...
- Test explicit rejection of sensitive keys
  - **Status:** [ ]
  - **Coverage notes:** Required explicitly to pass strict zero-trust enterprise auditing for $450M market adoption.
  - **Iteration delta:** Initial baseline
...
- Integrate HSM EV token
  - **Status:** [ ]
  - **Coverage notes:** Runs in parallel with Phase 4 testing but explicitly blocks final public release.
  - **Iteration delta:** Initial baseline
