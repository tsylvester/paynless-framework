...
is there a <li> command or style to make these bullets? 

Yes, absolutely! The standard HTML way to create bullet points is by using the <ul> (unordered list) tag, which you already have. The issue is that the default browser list styling might be overridden by your CSS framework (Tailwind CSS in this case), and you've manually added hyphens.
To get proper bullet points using Tailwind:
Remove the hyphens: Delete the - from the start of each list item's text.
Add Tailwind list classes: Add classes to the <ul> tag to style it as a bulleted list. The most common classes are list-disc (for the bullet style) and list-inside (to position the bullet).
Here's how you can modify that section in apps/web/src/pages/Home.tsx:
Current Code (Simplified):
Apply to README.md
Proposed Change:
Apply to README.md
We added list-disc to the <ul> to get the bullet style.
We added list-inside to position the bullet within the text block.
(Optional) I added space-y-1 mt-1 for a little vertical spacing between items and the top margin, which often looks better.
Crucially, the - text was removed from inside each <li>.
Would you like me to apply this change to apps/web/src/pages/Home.tsx?
You said we would Confirm Function Name: What is the exact name of your AI chat Edge Function directory (e.g., chat-ai, chat, send-message)?
