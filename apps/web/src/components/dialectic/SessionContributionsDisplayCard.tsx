...
import { Loader2, MoreVertical, Download, Send } from "lucide-react";
...
export const SessionContributionsDisplayCard: React.FC = () => {
	...
	const activeStage = useMemo(() => {
		return (
			processTemplate?.stages?.find((s) => s.slug === activeStageSlug) || null
		);
	...
	const displayedContributions = useMemo(() => {
		...
		const contributionsForStageAndIteration =
			session.dialectic_contributions.filter(
				(c) =>
		...
		for (const contrib of contributionsForStageAndIteration) {
			...
			if (!existing || contrib.edit_version > existing.edit_version) {
				...
				contrib.edit_version === existing.edit_version &&
				contrib.is_latest_edit
			) {
				latestEditsMap.set(originalId, contrib);
			}
		}
		for (const contrib of contributionsForStageAndIteration) {
			...
			if (!latestEditsMap.has(originalId)) {
				...
				if (
					!currentInMap.is_latest_edit &&
					contrib.id === originalId &&
					contrib.is_latest_edit
				) {
					latestEditsMap.set(originalId, contrib);
				}
		...
		contributionsForStageAndIteration.forEach((c) => {
			if (c.is_latest_edit) {
				originalIdsWithAnyLatestEdit.add(
					c.original_model_contribution_id || c.id,
				);
		...
		const uniqueFinalContributions = Array.from(
			new Map(finalContributions.map((c) => [c.id, c])).values(),
		);

		// --- Placeholder Logic ---
		if (isGenerating && selectedModelIds.length > 0) {
			const placeholders: DialecticContribution[] = [];
			const receivedModelIds = new Set(
				uniqueFinalContributions.map((c) => c.model_id),
			);

			selectedModelIds.forEach((modelId) => {
				if (!receivedModelIds.has(modelId)) {
					const modelInfo = modelCatalog.find(
						(m: AIModelCatalogEntry) => m.id === modelId,
					);
					placeholders.push({
		...
		return uniqueFinalContributions.sort((a, b) =>
			(a.model_name || "").localeCompare(b.model_name || ""),
		);
	...
	const handleResponseChange = (
		...
		setStageResponses((prev) => ({
			...prev,
			[originalModelContributionId]: responseText,
		}));
		setSubmissionSuccessMessage(null);
		if (submissionError) {
			resetSubmitError?.();
	...
	const handleSubmitResponses = async () => {
		...
		const hasAnyResponses = Object.values(stageResponses).some(
			(text) => text && text.trim() !== "",
		);
				...
				{displayedContributions.length === 0 && !isGenerating && (
					<div className="text-center py-16">
						...
						<p className="text-muted-foreground max-w-md mx-auto">
							Start by clicking the Generate button to create AI contributions for this stage
						</p>
					</div>
				)}
				{isGenerating &&
					...
					Array.from({ length: 2 }).map((_, index) => (
						<GeneratedContributionCardSkeleton key={index} />
					))}
