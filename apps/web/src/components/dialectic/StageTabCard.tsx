...
type StageProgressSummary = ReturnType<typeof selectStageProgressSummary>;

const emptyStageProgressSummary: StageProgressSummary = {
  isComplete: false,
  totalDocuments: 0,
...
const StageCard: React.FC<StageCardProps> = ({ stage }) => {
  ...
  const stageProgressSummary = useDialecticStore((state) => {
    if (!session) {
      return emptyStageProgressSummary;
    }
  ...
  const hasDocumentSummary = stageProgressSummary.totalDocuments > 0;
  const showCompletedLabel = (hasDocumentSummary && stageProgressSummary.isComplete)
    || (!hasDocumentSummary && Boolean(contributionsForStageExist));
    ...
    >
      ...
      {hasDocumentSummary && (
        <p className="text-xs text-muted-foreground text-center mt-1">
          {stageProgressSummary.completedDocuments} / {stageProgressSummary.totalDocuments} documents
        </p>
      )}
    </Card>
  );
