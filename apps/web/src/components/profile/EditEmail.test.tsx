...
describe('EditEmail Component', () => {
  ...
  it('should render with initial email in an input field and a disabled Save button', () => {
    ...
    render(<EditEmail />);
    expect(screen.getByTestId('shadcn-card-title')).toHaveTextContent('Edit Email');
  ...
  it('should call updateEmail on Save and show success toast', async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateEmail).toHaveBeenCalledWith({ email: 'updated@example.com' });
      expect(toast.success).toHaveBeenCalledWith('Email update request sent! Check your inbox for verification.');
    });
  ...
  it('should display loading state on Save button during save operation', async () => {
    ...
    setupMockStore(mockAuthUserInitial, mockUserProfile, false, () => updatePromise);
    ...
    await act(async () => { 
      ...
      setupMockStore(updatedUserAfterSaveAttempt, mockUserProfile, false, () => updatePromise);
    });

    const finalSaveButton = screen.getByRole('button', { name: /save changes/i });
  ...
  it('should show error toast and in-card error if updateEmail returns false (simulating API error)', async () => {
    ...
    setupMockStore(mockAuthUserInitial, mockUserProfile, false, new Error(errorMsg), mockUpdateEmail);
    ...
    await waitFor(() => {
      expect(mockUpdateEmail).toHaveBeenCalledWith({ email: 'error@example.com' });
    });

    await waitFor(() => {
        expect(screen.getByTestId('alert-circle-icon')).toBeInTheDocument();
        expect(screen.getByText(errorMsg)).toBeInTheDocument();
        expect(toast.error).toHaveBeenCalledWith('Failed to update email. An unexpected error occurred.');
     });
     const saveButtonAfterError = screen.getByRole('button', { name: /save changes/i });
  ...
  it('should show user data not available if user is not loaded', () => {
    ...
    render(<EditEmail />);
    expect(screen.getByText(/User data not available. Cannot edit email./i)).toBeInTheDocument();
