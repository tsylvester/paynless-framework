...
export const TokenUsageDisplay: React.FC<TokenUsageDisplayProps> = ({ tokenUsage }) => {
  if (!tokenUsage || 
      typeof tokenUsage.prompt_tokens === 'undefined' || 
      typeof tokenUsage.completion_tokens === 'undefined' ||
      typeof tokenUsage.total_tokens === 'undefined'
     ) {
  ...
  return (
    <div className="text-xs text-muted-foreground" data-testid="token-usage-display">
      {`P:${tokenUsage.prompt_tokens} / C:${tokenUsage.completion_tokens} / T:${tokenUsage.total_tokens}`}
    </div>
  );
}; 