...
export const MessageSelectionControls: React.FC = () => {
  ...
  const handleCheckedChange = () => {
    ...
    } else { // 'none' or 'some' or 'empty' (if empty, selectAll will do nothing gracefuly)
  ...
  return (
    <div className="flex items-center space-x-2">
      <Checkbox
        ...
        disabled={isDisabled}
        aria-label={selectionState === 'all' ? "Deselect all messages" : "Select all messages"}
      />
      <Label htmlFor="select-all-messages-checkbox" className={`text-sm ${isDisabled ? 'text-muted-foreground' : ''}`}>
        {selectionState === 'all' ? 'All' : selectionState === 'some' ? 'Some' : 'None'}
      </Label>
    </div>
  );
}; 