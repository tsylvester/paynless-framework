...
function AppContent() {
	...
	const setShowWelcomeModal = useAuthStore(
		(state) => state.setShowWelcomeModal,
	);

	const { loadPersonalWallet, personalWallet, isLoadingPersonalWallet } =
	...
	useEffect(() => {
		console.log(
			"[AppContent useEffect] Triggered. Profile:",
			JSON.stringify(profile, null, 2),
		);
		console.log(
			"[AppContent useEffect] Triggered. isChatContextHydrated:",
			isChatContextHydrated,
		);
		if (profile) {
			...
			if (!isChatContextHydrated) {
				logger.info(
					"[AppContent] Profile available and AI chat context not hydrated. Hydrating...",
				);
				hydrateChatContext(
					isChatContextPreferences(profile.chat_context)
			...
			if (profile.has_seen_welcome_modal === false) {
				console.log(
					"[AppContent useEffect] Condition met: profile.has_seen_welcome_modal is false. Calling setShowWelcomeModal.",
				);
				logger.info(
					"[AppContent] New user profile detected, showing welcome modal.",
				);
			...
			logger.info(
				"[AppContent] Profile removed (logout) and AI chat context was hydrated. Resetting AI context...",
			);
			resetChatContextToDefaults();
		}
	}, [
					...
					<Alert variant="destructive">
						...
						<AlertDescription>
							{capabilityError.message ||
								"Failed to load platform features. Some functionality may be limited."}
						</AlertDescription>
					</Alert>
				</div>
			)}
