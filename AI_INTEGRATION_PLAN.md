...
4.  **(TDD):** Write tests for RLS policies if possible within your testing framework.

**Phase 2: Backend (Supabase Edge Functions)**

1.  **New Environment Variables:** Add necessary API keys (e.g., `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) securely to Supabase environment variables (Vault or Project Settings > Functions). **CRITICAL: Never store API keys directly in the `ai_providers` table.**
2.  **`ai-providers` Function:**
    ...
    *   **(TDD):** Unit test handler logic (mock Supabase client).
3.  **`system-prompts` Function:**
    ...
    *   **(TDD):** Unit test handler logic.
4.  **`chat` Function (Core Interaction):**
    ...
    *   **(TDD):** Unit test handler logic extensively (mock Supabase client, mock external AI API calls, test history reconstruction, test saving logic, test auth/anonymous checks).
