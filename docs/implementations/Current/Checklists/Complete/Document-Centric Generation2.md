...
*   `[✅]` 1.b. `[TYPES]` Define stage recipe contracts for the frontend.
    *   `[✅]` 1.b.i. Create precise recipe types in `@paynless/types` aligned with backend (`dialectic_stage_recipe_steps`).
        ...
        *   `[✅]` 1.b.i.b. Add `DialecticStageRecipeStep` with fields: `id`, `step_key`, `step_slug`, `step_name`, `execution_order`, `parallel_group?`, `branch_key?`, `job_type ('PLAN'|'EXECUTE'|'RENDER')`, `prompt_type ('Planner'|'Turn')`, `prompt_template_id?`, `output_type ('HeaderContext'|'AssembledDocumentJson'|'RenderedDocument')`, `granularity_strategy ('all_to_one'|'per_source_document'|'one_to_many'|'many_to_one')`, `inputs_required: InputRequirement[]`, `inputs_relevance?: InputsRelevanceItem[]`, `outputs_required?: OutputRequirement[]`.
