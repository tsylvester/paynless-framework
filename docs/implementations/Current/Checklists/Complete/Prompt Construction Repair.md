...
*   `[ ]` 10.h. `[REFACTOR]` **Refactor `AiServiceFactory` for True, Complete Dependency Injection**
    ...
    *   `[ ]` 10.l. `[REFACTOR]` **Refactor Embedding Service and Test Harness for True Dependency Injection**
        ...
        *   `[✅]` 10.l.iii. `[REFACTOR]` **Make `DummyAdapter` Conform to the New Contract**
            ...
            *   `[✅]` 10.l.iii.2. `[BE]` **GREEN: Implement Fix:** In `supabase/functions/_shared/ai_service/dummy_adapter.ts`, add a `getEmbedding` method. This method will return a promise that resolves to a mock embedding response (e.g., `{ data: [{ embedding: [0.1, 0.2, 0.3] }] }`), matching the structure the `EmbeddingClient` expects.
            *   `[✅]` 10.l.iii.3. `[TEST-UNIT]` **PROVE: Confirm Fix:** Rerun the test from the RED step and confirm it now passes.
        *   `[✅]` 10.l.iv. `[REFACTOR]` **Decouple `EmbeddingClient` from the Concrete Adapter**
            *   `[✅]` 10.l.iv.1. `[TEST-UNIT]` **RED: Prove Flaw:** In a test file for `indexing_service.ts`, write a test that attempts to instantiate `EmbeddingClient` with a `DummyAdapter` instance. This will fail the type-check because the constructor requires a specific `OpenAiAdapter`.
            *   `[✅]` 10.l.iv.2. `[BE]` **GREEN: Implement Fix:** In `supabase/functions/_shared/services/indexing_service.ts`, change the constructor of `EmbeddingClient` to accept a generic `AiProviderAdapterInstance` instead of a concrete `OpenAiAdapter`.
        ...
        *   `[✅]` 10.l.v. `[REFACTOR]` **Fix Embedding Client Instantiation in Test Harness**
            ...
            *   `[✅]` 10.l.v.5. `[TEST-INT]` This generic adapter instance will then be passed to the `EmbeddingClient` constructor, satisfying its dependency without using a real adapter.
