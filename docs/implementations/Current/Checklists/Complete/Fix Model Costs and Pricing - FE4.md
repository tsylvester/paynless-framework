...
* **Cost ceiling fix-forward — global contracts (all nodes in this chain through commit)**
  ...
  * **Cap init vs loading:** `initializeMaxOutputTokens(): InitializeMaxOutputTokensResult` — **silent skip** (no store write, no error return) while any cap-init dependency is still loading or `outputCapUserCustomized === true`. When deps ready and init fails, **return** `{ ok: false, error: ApiError }` — log at error level, **never** write error to dialectic store. Origin codes: `NO_DEFAULT_GENERATION_MODELS`, `MODEL_CATALOG_ENTRY_MISSING`, `MODEL_OUTPUT_CAP_UNAVAILABLE`, `MODEL_CATALOG_INVALID_CONFIG`. On success, `set({ maxOutputTokens })` only (never `setMaxOutputTokens`). **Post-project (session loaded):** dialectic store orchestrator invokes init on catalog fetch success, session hydration, and deep-link — UI mount is not required for bootstrap. **Pre-project (no session/project context):** `CreateDialecticProjectForm`, `CreateProjectFromChatButton`, and `chat/index` invoke when `isCapInitReady` and capture return in **component-local** `capInitResult` for footer/toast gates (not Zustand). **SessionInfoCard (Option B):** reconciliation call only — after store orchestrator and `!isCostEstimateLoading`, when cap still not finite, call `initializeMaxOutputTokens()` to populate `capInitResult` for verbose init error pass-through before selector; not responsible for catalog fetch or hydration timing. UI must surface **every** `{ ok: false, error }` code above before selector when tier/auth gates pass.
  * **Fix-forward sequence (store-driven cap init on session entry):** `packages/store/src/dialecticStore` → `useStartContributionGeneration` → `GenerateContributionButton` (integration/regression) → `SessionInfoCard`. Pre-project nodes retain mount init; amend only if regression.
...
* `[✅]`   packages/store/src/dialecticStore **Initialize maxOutputTokens from min(tierCap, bindingModelCap); guard catalog config; cap-init waits for deps; store-driven post-project orchestration; pre-project fetchProcessTemplate silence; rules-compliant error return/rethrow (no stored errors)**
  ...
  * `[✅]`   `objective`
    ...
    * `[✅]`   **Functional goal:** `initializeMaxOutputTokens` never uses level-0 tier fallback. Pre-project UI nodes call this action when cap-init deps are ready; post-project paths invoke via store orchestrator below; this node does not edit UI consumers.
    * `[✅]`   **Problem (fix-forward):** Session/deep-link entry (`activateProjectAndSessionContextForDeepLink`, sidebar, notifications) never fetches model catalog and never runs cap init after hydration. `maxOutputTokens` stays `null` while selectors return `OUTPUT_CAP_NOT_INITIALIZED`. Consumer `useEffect` bootstrap races hydration and misses catalog-empty paths.
    * `[✅]`   **Functional goal — internal orchestrator (post-project only):** Add internal `initializeOutputCap` in `dialecticStore.ts` (not exported on `DialecticActions`): (1) when `modelCatalog.length === 0 && !isLoadingModelCatalog`, `await fetchAIModelCatalog()` (mirror `createProjectAndAutoStart` catalog guard); (2) when cap-init deps ready (`!useAuthStore.getState().isLoading`, `userTier !== null`, `!isLoadingModelCatalog`, `modelCatalog.length > 0`, `!outputCapUserCustomized`), call `initializeMaxOutputTokens()`; (3) log init failures at error level; never write init errors to dialectic store.
    * `[✅]`   **Functional goal — orchestrator invocation sites (ordering):** Invoke orchestrator at end of `fetchAIModelCatalog` success; at end of `fetchAndSetCurrentSessionDetails` **after** `set({ selectedModels: models })` from session response (init must not run before session models are set); inside `activateProjectAndSessionContextForDeepLink` ensure catalog fetch + init complete as part of thunk (catalog before or in parallel with session fetch, init after session models applied).
    * `[✅]`   **Functional goal — `maxOutputTokens` reset on context change:** Extend `setActiveDialecticContext` when `projectId` or `sessionId` changes: set `maxOutputTokens: null` alongside existing `outputCapUserCustomized: false` reset so stale caps cannot mask missing re-init.
  ...
  * `[✅]`   `role`
    * `[✅]`   Store layer — owns `maxOutputTokens` and `outputCapUserCustomized`; exposes `initializeMaxOutputTokens` for pre-project UI callers and idempotent re-entry.
    * `[✅]`   Post-project cap-init orchestration on session/project hydration and deep-link; pre-project UI retains explicit init (no session row to hydrate).
  ...
  * `[✅]`   packages/store/src/dialecticStore.session.test.ts
    ...
    * `[✅]`   `setActiveDialecticContext` new `sessionId` or `projectId` resets `maxOutputTokens` to `null` alongside `outputCapUserCustomized: false`.
    * `[✅]`   `fetchAndSetCurrentSessionDetails` success with valid tier + catalog + session `selected_models` → finite `maxOutputTokens` after thunk completes (orchestrator ran after models set).
    * `[✅]`   `activateProjectAndSessionContextForDeepLink` with empty initial `modelCatalog` → after thunk, catalog loaded and `maxOutputTokens` finite when tier + session models + fixture catalog valid.
    * `[✅]`   `activateProjectAndSessionContextForDeepLink` — init runs after `selectedModels` from session, not before (seed empty `selectedModels` mid-fetch; assert defaults not applied over session models).
    * `[✅]`   orchestrator does not overwrite cap when `outputCapUserCustomized: true`.
  ...
  * `[✅]`   packages/store/src/dialecticStore.ts
    ...
    * `[✅]`   implement internal `initializeOutputCap` per objective; wire invocation sites on `fetchAIModelCatalog` success, `fetchAndSetCurrentSessionDetails` after `selectedModels` set, and `activateProjectAndSessionContextForDeepLink`.
    * `[✅]`   extend `setActiveDialecticContext` context-change branch to reset `maxOutputTokens: null` when `projectId` or `sessionId` changes.
  ...
  * `[✅]`   apps/web/src/mocks/dialecticStore.mock.ts
    ...
    * `[✅]`   `fetchAIModelCatalog` mock invokes cap-init orchestrator semantics on success when test exercises post-project path (or document exemption when mock cannot mirror async orchestrator — integration tests use real store).
  ...
  * `[✅]`   `requirements`
    ...
    * `[✅]`   `activateProjectAndSessionContextForDeepLink` with empty catalog seeds finite `maxOutputTokens` when fixtures valid.
    * `[✅]`   `fetchAndSetCurrentSessionDetails` sets finite `maxOutputTokens` after session `selected_models` applied.
    * `[✅]`   Context change (`projectId` or `sessionId`) sets `maxOutputTokens: null`.
    * `[✅]`   `selectCostCeiling` on store state after deep-link hydration does not return `OUTPUT_CAP_NOT_INITIALIZED` when tier + catalog + session models + counts/template fixtures satisfy node 2 success preconditions (session test or integration seed).
...
* `[✅]`   apps/web/src/components/dialectic/CreateDialecticProjectForm **Tier cap autostart bootstrap; cost-ceiling UI from loading flags and pass-through errors; compact formatTokenCount display; integration test without popover**
  ...
  * `[✅]`   `requirements`
    ...
    * `[✅]`   **Pre-project entry (fix-forward):** Retain mount `useEffect` cap init when `isCapInitReady` — no session/project hydration path exists on create-project form. All existing autostart, integration, and cost-ceiling tests in this node remain green after store orchestration lands.

* `[✅]`   apps/web/src/components/dialectic/SessionInfoCard **Session cost-ceiling UI from loading flags and pass-through errors; Option B reconciliation capInitResult; compact formatTokenCount display**
  ...
  * `[✅]`   `objective`
    ...
    * `[✅]`   **Functional goal:** Import `useAuthStore` from `@paynless/store`. Subscribe `isLoading`, `userTier`. Subscribe `initializeMaxOutputTokens` from `useDialecticStore`. Define **`isCapInitReady`** when **all** of: `isLoading === false`; `userTier !== null`; `isLoadingModelCatalog === false`; `modelCatalog.length > 0`.
    * `[✅]`   **Problem (fix-forward):** Mount `useEffect` bootstrap cap init duplicates store orchestrator on session entry; races selectors before hydration completes; does not fetch catalog on deep-link-only paths.
    * `[✅]`   **Functional goal — remove bootstrap init:** Delete mount `useEffect` that calls `initializeMaxOutputTokens()` whenever `isCapInitReady && session !== null`. Post-project cap bootstrap is owned by `packages/store/src/dialecticStore` orchestrator. Component does not call `fetchAIModelCatalog`.
    * `[✅]`   **Functional goal — Option B reconciliation `capInitResult` (verbose init errors):** Add `useEffect` with deps `[isLoading, userTier, isLoadingModelCatalog, modelCatalog.length, session?.id, selectedModels.length, isCostEstimateLoading, maxOutputTokens, outputCapUserCustomized]`: when `isCapInitReady && session !== null && !isCostEstimateLoading && !outputCapUserCustomized && (maxOutputTokens === null || !Number.isFinite(maxOutputTokens))`, call `initializeMaxOutputTokens()` and assign return to component-local `capInitResult` (clear on `{ ok: true }` or deps change). Purpose: surface **every** init failure `error.message` verbatim in footer gate step (3) before selector `OUTPUT_CAP_NOT_INITIALIZED` — not responsible for catalog fetch or hydration timing. Cap init must not depend on `isModelSelectorOpen` or popover mount.
    ...
    * `[✅]`   **Functional goal — extend loading flags (fix-forward, parity with hook):** Add `isLoadingModelCatalog` and `isLoadingActiveSessionDetail` to `isCostEstimateLoading`. Extend first-match loading copy order: tier → `"Loading subscription tier…"`; catalog → `"Loading model catalog…"`; session → `"Loading session…"`; template → `"Loading process template…"`; progress pending → `"Loading stage progress…"`. While any loading flag true, do not render error or success estimate lines; reconciliation `useEffect` must not run during loading (`!isCostEstimateLoading` guard).
  ...
  * `[✅]`   `role`
    * `[✅]`   Session header UI — displays session cost ceiling from `selectCostCeiling` + hydration/template/catalog loading flags; Option B reconciliation for verbose init errors.
    * `[✅]`   Out of scope: post-project cap bootstrap (dialecticStore orchestrator), generate button NSF, slider UX, selector implementation, pre-project form, subscription cart prefill.

  * `[✅]`   `module`
    * `[✅]`   `apps/web` dialectic session info card and `SessionInfoCard.test.tsx` only.

  * `[✅]`   `deps`
    * `[✅]`   `packages/store/src/dialecticStore`: post-project cap-init orchestrator on session/deep-link hydration.
    * `[✅]`   Node 1: `initializeMaxOutputTokens` on `useDialecticStore` (reconciliation call only).
  ...
  * `[✅]`   `context_slice`
    * `[✅]`   Reads: `activeSessionDetail`, `currentProjectDetail`, auth tier + loading + `error`, component-local `capInitResult`, `progressHydrationStatus`, `isLoadingProcessTemplate`, `maxOutputTokens`, `outputCapUserCustomized`, `selectCostCeiling(session.id)`.
    * `[✅]`   Writes: reconciliation `initializeMaxOutputTokens` call only when cap still not finite after loading (no direct `setMaxOutputTokens`).
  ...
  * `[✅]`   apps/web/src/components/dialectic/SessionInfoCard.test.tsx
    ...
    * `[✅]`   replace test `calls initializeMaxOutputTokens when auth tier and catalog loaded and session active` — rename `reconciliation calls initializeMaxOutputTokens when cap still null after loading and store orchestrator skipped or failed` — seed `maxOutputTokens: null`, `outputCapUserCustomized: false`, `!isCostEstimateLoading`, valid session; expect reconciliation spy called; assign `capInitResult` on `{ ok: false, error }`.
    * `[✅]`   add test `does not call initializeMaxOutputTokens for bootstrap when maxOutputTokens already finite from store` — seed finite `maxOutputTokens` from fixture; render; reconciliation spy not called (or called zero times for bootstrap path — only when cap null after loading per Option B).
    * `[✅]`   add test `shows catalog loading notice while isLoadingModelCatalog without OUTPUT_CAP_NOT_INITIALIZED error` — `isLoadingModelCatalog: true`; selector mock `OUTPUT_CAP_NOT_INITIALIZED`; expect `session-info-estimate-loading-notice` `"Loading model catalog…"`; no error notice.
    * `[✅]`   add test `shows session loading notice while isLoadingActiveSessionDetail` — `isLoadingActiveSessionDetail: true`; selector mock `OUTPUT_CAP_NOT_INITIALIZED`; expect loading notice `"Loading session…"`; no error notice; reconciliation not called.
  ...
  * `[✅]`   apps/web/src/components/dialectic/SessionInfoCard.tsx
    ...
    * `[✅]`   remove bootstrap cap-init `useEffect`; implement Option B reconciliation `useEffect` per objective; subscribe `maxOutputTokens`, `outputCapUserCustomized`.
    * `[✅]`   extend `isCostEstimateLoading` and loading-notice first-match order to include `isLoadingModelCatalog` per fix-forward objective.
  ...
  * `[✅]`   `requirements`
    ...
    * `[✅]`   With finite `maxOutputTokens` from store after hydration: reconciliation `initializeMaxOutputTokens` not called on mount; footer success or loading, not bootstrap `OUTPUT_CAP_NOT_INITIALIZED` flash.
    * `[✅]`   With `maxOutputTokens: null` after `!isCostEstimateLoading`: reconciliation invokes `initializeMaxOutputTokens`; `capInitResult` errors surface verbatim before selector.
...
* `[✅]`   apps/web/src/components/dialectic/OutputCapSlider **Blocked/loading UI instead of silent null; live header display during drag; store SSOT for selectors; persist cap only on user commit**
  ...
  * `[✅]`   `objective`
    ...
    * `[✅]`   **Functional goal — no init in this component:** Do not import or call `initializeMaxOutputTokens`. Do not call `setMaxOutputTokens` in `useEffect`, on mount, or when syncing display from tier/model cap. Pre-project parents (`CreateDialecticProjectForm`) and post-project store orchestrator + `SessionInfoCard` reconciliation own cap init; slider reads store SSOT only.
...
* `[✅]`   apps/web/src/components/chat/index.tsx **Chat onboarding pre-project cost estimate; sync models to dialectic store; cap init when deps ready; compact formatTokenCount display**
  ...
  * `[✅]`   `requirements`
    ...
    * `[✅]`   **Pre-project entry (fix-forward):** Retain mount `useEffect` cap init when `isCapInitReady && selectedDomainId && hasSelectedModel` — chat onboarding has no session/project hydration. All existing `index.test.tsx` tests remain green after store orchestration lands.

* `[✅]`   apps/web/src/hooks/useStartContributionGeneration **ComputeCostCeilingReturn-only selector consumption; loading gates before cost errors; catalog and cap-init-pending gates; pass-through ApiError on spend guard**
  ...
  * `[✅]`   `objective`
    ...
    * `[✅]`   **Problem (fix-forward):** `isCostEstimateLoading` omits `isLoadingModelCatalog` and cap-init pending window — sidebar `GenerateContributionButton` surfaces `OUTPUT_CAP_NOT_INITIALIZED` while store orchestrator is still fetching catalog or initializing cap.
    * `[✅]`   **Functional goal — extend loading before errors:** Subscribe `isLoadingModelCatalog`, `isLoadingActiveSessionDetail` from dialectic store. Add to `isCostEstimateLoading` when **any** of: `isLoadingModelCatalog === true`; `isLoadingActiveSessionDetail === true` (store orchestrator has not finished session hydration + cap init). **Do not** treat `maxOutputTokens === null` alone as loading after catalog and session fetch complete — init failures must surface as `costCeilingError` / pass-through selector or toast after loading clears. Extend `startContributionGeneration` loading guard and re-read at call time to include same flags; toast `"Loading cost estimate…"` when any true. While loading, same suppress-error semantics as existing objective.
    ...
    * `[✅]`   **Non-functional:** Do not edit `GenerateContributionButton.tsx`, `dialecticStore.selectors.ts`, `SessionInfoCard.tsx`, or subscription CTAs in this node. Depends on nodes 1–2 (cap SSOT + loud selectors). Cap init is not duplicated in this hook.
  ...
  * `[✅]`   `role`
    ...
    * `[✅]`   Out of scope: button markup/copy (`GenerateContributionButton` next node); store selector implementation; cap-init orchestration (dialecticStore node).
  ...
  * `[✅]`   `deps`
    ...
    * `[✅]`   `packages/store/src/dialecticStore`: post-project cap-init orchestrator sets `maxOutputTokens` before consumers read selector.
    * `[✅]`   `useAuthStore` — `isLoading`, `userTier`.
    * `[✅]`   `useDialecticStore` — add `isLoadingModelCatalog`, `isLoadingActiveSessionDetail`; retain `isLoadingProcessTemplate`, `progressHydrationStatus`, existing subscriptions.
  ...
  * `[✅]`   apps/web/src/hooks/useStartContributionGeneration.test.ts
    ...
    * `[✅]`   add test `while isLoadingModelCatalog, isCostEstimateLoading is true and costCeilingError is null` — seed `isLoadingModelCatalog: true`; selector mock `OUTPUT_CAP_NOT_INITIALIZED`; expect `isCostEstimateLoading` true, `costCeilingError` null, `showCostEstimateBlocked` false; `startContributionGeneration` toasts `"Loading cost estimate…"`.
    * `[✅]`   add test `while isLoadingActiveSessionDetail, isCostEstimateLoading is true and costCeilingError is null` — seed `isLoadingActiveSessionDetail: true`; selector mock `OUTPUT_CAP_NOT_INITIALIZED`; same loading assertions.
    * `[✅]`   add test `after session load completes with finite maxOutputTokens, loading clears and success exposes ceilings` — `isLoadingActiveSessionDetail: false`, finite cap; `isCostEstimateLoading` false; success path flags restore.
    * `[✅]`   add test `after session load completes with maxOutputTokens still null and init failed, loading clears and OUTPUT_CAP_NOT_INITIALIZED exposed` — `isLoadingActiveSessionDetail: false`, `isLoadingModelCatalog: false`, cap null; expect `costCeilingError` matches selector error; not perpetual loading.
  ...
  * `[✅]`   apps/web/src/hooks/useStartContributionGeneration.ts
    ...
    * `[✅]`   subscribe `isLoadingModelCatalog`, `isLoadingActiveSessionDetail`; extend `isCostEstimateLoading` and `startContributionGeneration` loading guard per fix-forward objective.
  ...
  * `[✅]`   `requirements`
    ...
    * `[✅]`   `isLoadingModelCatalog: true` with selector `OUTPUT_CAP_NOT_INITIALIZED` → loading only, no error exposed.
    * `[✅]`   `isLoadingActiveSessionDetail: true` → loading only; `false` with cap null → selector/init error exposed, not perpetual loading.
...
* `[✅]`   apps/web/src/components/dialectic/GenerateContributionButton **Remove bundled no-estimate UI; loading callout from hook; pass-through estimate errors only; compact formatTokenCount display; deep-link integration regression**
  ...
  * `[✅]`   `deps`
    ...
    * `[✅]`   `packages/store/src/dialecticStore`: deep-link/session hydration sets `maxOutputTokens` (integration tests seed via `activateProjectAndSessionContextForDeepLink` or equivalent, not component bootstrap).
  ...
  * `[✅]`   apps/web/src/components/dialectic/GenerateContributionButton.costCeiling.integration.test.tsx
    * `[✅]`   **Mock deps:** Extend test setup to seed auth tier (`useAuthStore` mock or existing pattern in file): `isLoading: false`, `userTier.output_cap_tokens` matching fixture `maxOutputTokens`. In success-path `beforeEach` / `seedSessionStore`, **omit** manual `maxOutputTokens` override; after seed invoke store hydration path (`activateProjectAndSessionContextForDeepLink` or `fetchAndSetCurrentSessionDetails` + catalog fixtures) and `waitFor` finite `maxOutputTokens` from orchestrator — not manual `initializeMaxOutputTokens()` in test harness.
    ...
    * `[✅]`   success-stack test (`hydrate → stage/project ceiling → enabled generate`) passes with store orchestrator path instead of manual cap seed.
    * `[✅]`   add test `deep-link with empty initial catalog shows loading then success without persistent OUTPUT_CAP_NOT_INITIALIZED` — seed empty `modelCatalog`, MSW catalog + session fixtures; render button; expect `generate-button-estimate-loading-notice` during catalog load; after orchestrator, success callouts or enabled generate; never persistent `OUTPUT_CAP_NOT_INITIALIZED` with valid fixtures.
  ...
  * `[✅]`   apps/web/src/components/dialectic/GenerateContributionButton.integration.test.tsx
    ...
    * `[✅]`   in `setStoreForButton`, omit manual `maxOutputTokens` override on success paths; after `setDialecticStateValues`, invoke store orchestrator path (session hydration + catalog fixtures) and `waitFor` finite cap before assertions that require enabled generate/resume.
  ...
  * `[✅]`   `requirements`
    ...
    * `[✅]`   Deep-link integration: empty catalog → loading notice → orchestrator → enabled generate with valid fixtures.
...
* `[✅]`   apps/web/src/components/ai/CreateProjectFromChatButton **Tier cap init; ComputeCostCeilingReturn-only click gate; loading before selector errors; pass-through toast**
  ...
  * `[✅]`   `requirements`
    ...
    * `[✅]`   **Pre-project entry (fix-forward):** Retain mount and click-time `initializeMaxOutputTokens` on autostart path — no session/project hydration before `createProjectAndAutoStart`. All existing unit and integration tests in this node remain green after store orchestration lands.
  ...
  * `[✅]`   **Commit** `fix(dialectic): store-driven cap init on session entry — orchestrator, loading gates, test reconciliation`
    * `[✅]`   Store: internal `initializeOutputCap` on catalog success, session hydration after `selectedModels` set, and deep-link; `maxOutputTokens: null` reset on context change.
    * `[✅]`   Hook: `isLoadingModelCatalog` and `isLoadingActiveSessionDetail` in `isCostEstimateLoading`; sidebar estimate UX does not flash `OUTPUT_CAP_NOT_INITIALIZED` during catalog/session hydration pending.
    * `[✅]`   SessionInfoCard: bootstrap cap-init `useEffect` removed; Option B reconciliation preserves verbose `capInitResult` init errors before selector; catalog in loading flags.
    * `[✅]`   GenerateContributionButton: integration tests seed finite cap via store hydration path; deep-link empty-catalog regression green.
    * `[✅]`   Pre-project surfaces (`CreateDialecticProjectForm`, `CreateProjectFromChatButton`, `chat/index`): mount init retained; all existing tests green.
