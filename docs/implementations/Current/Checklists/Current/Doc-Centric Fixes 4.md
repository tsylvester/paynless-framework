...
*   `[ ]` 51. **`[BE]` Update processJob to Accept IJobContext and Slice Contexts**
    ...
    *   `[ ]` 51.b. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/processJob.ts`, implement context slicing:
        *   `[ ]` 51.b.i. Add imports: `import { IJobContext } from './JobContext.interface.ts';` and `import { createExecuteJobContext, createPlanJobContext, createRenderJobContext } from './createJobContext.ts';`
        *   `[ ]` 51.b.ii. Update function signature: `export async function processJob(dbClient: SupabaseClient<Database>, job: DialecticJobRow, projectOwnerUserId: string, processors: IJobProcessors, ctx: IJobContext, authToken: string): Promise<void>` (was `deps: IDialecticJobDeps`, now `ctx: IJobContext`).
        *   `[ ]` 51.b.iii. Replace all `deps` references with `ctx` in logging/error handling.
        *   `[ ]` 51.b.iv. Update processor dispatch logic to slice contexts before calling:
            ```typescript
            ...
            ```
    *   `[ ]` 51.c. `[TEST-UNIT]` **GREEN**: In `supabase/functions/dialectic-worker/processJob.test.ts`, update to test context slicing:
        *   `[ ]` 51.c.i. Import `createJobContext`, `IJobContext`, and slicer functions.
        *   `[ ]` 51.c.ii. Update mock construction: Create full `IJobContext` via `createJobContext(mockParams)`.
        *   `[ ]` 51.c.iii. Update `processJob(...)` calls to pass `ctx: IJobContext` as fifth parameter.
        *   `[ ]` 51.c.iv. Add test "slices to IExecuteJobContext for EXECUTE jobs": Mock processors, call `processJob` with EXECUTE job, spy on `processSimpleJob` call, assert it received `IExecuteJobContext` (verify fields like `ragService` present, `planComplexStage` absent).
        *   `[ ]` 51.c.v. Add test "slices to IPlanJobContext for PLAN jobs": Verify `processComplexJob` received only plan fields.
        *   `[ ]` 51.c.vi. Add test "slices to IRenderJobContext for RENDER jobs": Verify `processRenderJob` received only render fields.
        *   `[ ]` 51.c.vii. Re-run all `processJob.test.ts` tests, ensure they pass.
...
*   `[ ]` 52. **`[BE]` Update processSimpleJob to Accept IExecuteJobContext**
    *   `[ ]` 52.a. `[DEPS]` The `processJob` orchestrator now slices to `IExecuteJobContext` for EXECUTE jobs (from step 51). We need to update `processSimpleJob` in `supabase/functions/dialectic-worker/processSimpleJob.ts` to accept `ctx: IExecuteJobContext` instead of `deps: IDialecticJobDeps`. `processSimpleJob` executes EXECUTE jobs by calling `executeModelCallAndSave` (which accepts `IExecuteJobContext` from step 50) and handling results. After this change, TypeScript enforces that `processSimpleJob` cannot access plan utilities (`planComplexStage`) or render utilities (`documentRenderer`). This step updates: (1) Function signature to `ctx: IExecuteJobContext`, (2) All references from `deps` to `ctx`, (3) Tests to mock `IExecuteJobContext`.
    *   `[ ]` 52.b. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/processSimpleJob.ts`, update to use `IExecuteJobContext`:
        *   `[ ]` 52.b.i. Import `IExecuteJobContext` from `./types/JobContext.interface.ts`.
        *   `[ ]` 52.b.ii. Update function signature (line ~22): `export async function processSimpleJob(dbClient: SupabaseClient<Database>, job: Job & { payload: DialecticJobPayload }, projectOwnerUserId: string, ctx: IExecuteJobContext, authToken: string)` (was `deps: IDialecticJobDeps`, now `ctx: IExecuteJobContext`).
        *   `[ ]` 52.b.iii. Replace all `deps` references with `ctx`: `deps.logger` → `ctx.logger`, `deps.notificationService` → `ctx.notificationService`, `deps.promptAssembler` → `ctx.promptAssembler`, `deps.executeModelCallAndSave` → `ctx.executeModelCallAndSave`, etc.
        *   `[ ]` 52.b.iv. Verify function body does NOT access `planComplexStage` or `documentRenderer`. TypeScript will prevent this.
    *   `[ ]` 52.c. `[TEST-UNIT]` **GREEN**: In `supabase/functions/dialectic-worker/processSimpleJob.test.ts`, update mocks:
        *   `[ ]` 52.c.i. Import `createExecuteJobContext`, `createJobContext`, `IExecuteJobContext`.
        *   `[ ]` 52.c.ii. Update mock construction: Create root via `createJobContext(mockParams)`, slice: `const executeCtx = createExecuteJobContext(rootCtx);`.
        *   `[ ]` 52.c.iii. Update `processSimpleJob(...)` calls to pass `executeCtx: IExecuteJobContext` as fourth parameter.
        *   `[ ]` 52.c.iv. Re-run tests, ensure they pass.
    *   `[ ]` 52.d. `[LINT]` Run linter for `processSimpleJob.ts` and `processSimpleJob.test.ts`, resolve errors.
    *   `[ ]` 52.e. `[CRITERIA]` All requirements met: (1) `processSimpleJob` accepts `IExecuteJobContext`, (2) All refs updated to `ctx`, (3) Function cannot access plan/render fields (TypeScript enforced), (4) Tests mock sliced context, (5) Tests pass, (6) Files lint-clean.
...
*   `[ ]` 53. **`[BE]` Update processComplexJob to Accept IPlanJobContext**
    ...
    *   `[ ]` 53.b. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/processComplexJob.ts`, update to use `IPlanJobContext`:
        *   `[ ]` 53.b.i. Import `IPlanJobContext` from `./types/JobContext.interface.ts`.
        *   `[ ]` 53.b.ii. Update function signature: `export async function processComplexJob(dbClient: SupabaseClient<Database>, job: Job & { payload: DialecticPlanJobPayload }, projectOwnerUserId: string, ctx: IPlanJobContext, authToken: string)` (was `deps: IDialecticJobDeps`, now `ctx: IPlanJobContext`).
        *   `[ ]` 53.b.iii. Replace `deps` with `ctx`: `deps.logger` → `ctx.logger`, `deps.planComplexStage` → `ctx.planComplexStage`, etc.
        *   `[ ]` 53.b.iv. Update calls to `planComplexStage` to pass `ctx` instead of `deps`.
    *   `[ ]` 53.c. `[TEST-UNIT]` **GREEN**: In `processComplexJob.test.ts`, `processComplexJob.happy.test.ts`, `processComplexJob.errors.test.ts`, `processComplexJob.parallel.test.ts`, update mocks:
        *   `[ ]` 53.c.i. Import `createPlanJobContext`, `createJobContext`, `IPlanJobContext`.
        *   `[ ]` 53.c.ii. Update mock construction: Create root, slice: `const planCtx = createPlanJobContext(rootCtx);`.
        *   `[ ]` 53.c.iii. Update function calls to pass `planCtx: IPlanJobContext`.
        *   `[ ]` 53.c.iv. Re-run tests, ensure they pass.
...
*   `[ ]` 54. **`[BE]` Update planComplexStage to Accept IPlanJobContext**
    ...
    *   `[ ]` 54.b. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/task_isolator.ts`, update to use `IPlanJobContext`:
        *   `[ ]` 54.b.i. Import `IPlanJobContext` from `./types/JobContext.interface.ts`.
        *   `[ ]` 54.b.ii. Update `planComplexStage` function signature to accept `ctx: IPlanJobContext` (was `deps: IDialecticJobDeps`).
        *   `[ ]` 54.b.iii. Replace `deps` with `ctx` throughout function body.
    *   `[ ]` 54.c. `[TEST-UNIT]` **GREEN**: In `task_isolator.test.ts`, `task_isolator.planComplexStage.test.ts`, `task_isolator.parallel.test.ts`, update mocks to use `IPlanJobContext`. Re-run tests.
...
*   `[ ]` 55. **`[BE]` Update processRenderJob to Accept IRenderJobContext**
    ...
    *   `[ ]` 55.b. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/processRenderJob.ts`, update to use `IRenderJobContext`:
        *   `[ ]` 55.b.i. Import `IRenderJobContext` from `./types/JobContext.interface.ts`.
        *   `[ ]` 55.b.ii. Update function signature to accept `ctx: IRenderJobContext` (was `deps: IDialecticJobDeps`, now using hierarchical context).
        *   `[ ]` 55.b.iii. Replace `deps` with `ctx`. May need to destructure: `const { documentRenderer, logger, downloadFromStorage, fileManager, notificationService } = ctx;`.
    *   `[ ]` 55.c. `[TEST-UNIT]` **GREEN**: In `processRenderJob` test files (if exist), update mocks to use `IRenderJobContext`. Re-run tests.
