...
*   `[ ]` 117. **`[BE]` Update `assembleTurnPrompt` to follow canonical template resolution process using `document_template_id`**
    *   `[ ]` 117.a. `[DEPS]` The `assembleTurnPrompt` function in `supabase/functions/_shared/prompt-assembler/assembleTurnPrompt.ts` currently violates the canonical template resolution process by getting template filenames directly from `headerContext.files_to_generate[].template_filename` (lines 112-126) and downloading them from storage. The canonical process requires: (1) get the prompt name from `stage.recipe_step.prompt_template_id`, (2) query `system_prompts` table to get the database entry, (3) check for `document_template_id`, (4) use the template id to query `dialectic_document_templates` to get the storage address (`storage_bucket`, `storage_path`, `file_name`), (5) download the template from storage using the canonical database-driven path. The function depends on: `dbClient` (SupabaseClient) for database queries, `downloadFromStorage` (from `supabase_storage_utils.ts`) for file downloads, and `renderPrompt` function for prompt rendering. The function must be updated to follow the canonical process: query `system_prompts` using `stage.recipe_step.prompt_template_id`, get `document_template_id`, query `dialectic_document_templates` for storage location, download and decode the template, then use it instead of the header context filename.
    *   `[ ]` 117.b. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-assembler/assembleTurnPrompt.test.ts`, add a new failing test case that:
        *   `[ ]` 117.b.i. Mocks a `stage` object with a `recipe_step` containing `prompt_template_id: "prompt-id-456"`.
        *   `[ ]` 117.b.ii. Mocks a `job` payload with `header_context_resource_id: "header-resource-123"` and `document_key: "business_case"`.
        *   `[ ]` 117.b.iii. Mocks a header context download that returns a JSON blob with `files_to_generate` array (to satisfy existing header context parsing).
        *   `[ ]` 117.b.iv. Mocks a database query to `system_prompts` that returns `{ prompt_text: null, document_template_id: "template-uuid-456" }` when queried with `prompt_template_id: "prompt-id-456"`.
        *   `[ ]` 117.b.v. Mocks a `dialectic_document_templates` record with `id: "template-uuid-456"`, `storage_bucket: "prompt-templates"`, `storage_path: "docs/prompts/thesis/"`, `file_name: "thesis_business_case_turn_v1.md"`.
        *   `[ ]` 117.b.vi. Mocks `downloadFromStorage` to return template content as `ArrayBuffer` containing `"# Turn Template\n\nThis is the turn template."` when called with the canonical storage path.
        *   `[ ]` 117.b.vii. Invokes `assembleTurnPrompt` with the mocked dependencies.
        *   `[ ]` 117.b.viii. Asserts that `system_prompts` was queried using `stage.recipe_step.prompt_template_id` (proving canonical process is followed).
        *   `[ ]` 117.b.ix. Asserts that `dialectic_document_templates` was queried using the `document_template_id` from `system_prompts` (proving canonical process is followed).
        *   `[ ]` 117.b.x. Asserts that `downloadFromStorage` was called with `dbClient`, `"prompt-templates"`, and `"docs/prompts/thesis/thesis_business_case_turn_v1.md"` (the canonical storage path, not the header context filename).
        *   `[ ]` 117.b.xi. Asserts that `renderPrompt` was called with the downloaded template content `"# Turn Template\n\nThis is the turn template."` (not the header context filename or null).
        *   `[ ]` 117.b.xii. This test must fail because `assembleTurnPrompt` currently gets template filenames from `headerContext.files_to_generate[].template_filename` and downloads directly, bypassing the canonical database-driven process, proving the flaw.
    *   `[ ]` 117.c. `[BE]` In `supabase/functions/_shared/prompt-assembler/assembleTurnPrompt.ts`, update the function to follow the canonical template resolution process:
        *   `[ ]` 117.c.i. After fetching and parsing the header context (lines 78-108), instead of using `docInfo.template_filename` from `headerContext.files_to_generate` (lines 112-126), query `system_prompts` using `stage.recipe_step.prompt_template_id`: `.from("system_prompts").select("prompt_text, document_template_id").eq("id", stage.recipe_step.prompt_template_id).single()`.
        *   `[ ]` 117.c.ii. Check if `promptTemplateData.prompt_text` is null or empty. If it is null/empty AND `promptTemplateData.document_template_id` exists and is a non-empty string, proceed with template resolution from storage using the canonical process.
        *   `[ ]` 117.c.iii. If `document_template_id` is present, query `dialectic_document_templates` table using `.from("dialectic_document_templates").select("storage_bucket, storage_path, file_name").eq("id", promptTemplateData.document_template_id).single()` to get the template storage location.
        *   `[ ]` 117.c.iv. Validate that `storage_bucket`, `storage_path`, and `file_name` are all present and non-empty strings. If any are missing, throw an error: `"Invalid template record: missing storage_bucket, storage_path, or file_name"`.
        *   `[ ]` 117.c.v. Construct the full storage path by combining `storage_path` and `file_name`, ensuring no double slashes (remove trailing slash from `storage_path` if present, ensure `file_name` doesn't start with a slash): `const fullPath = \`${storage_path.replace(/\/$/, '')}/${file_name}\`;`.
        *   `[ ]` 117.c.vi. Call `downloadFromStorage(dbClient, storage_bucket, fullPath)` to download the template file using the canonical database-driven path.
        *   `[ ]` 117.c.vii. If the download fails (`error` is not null) or returns no data (`data` is null), throw an error: `"Failed to download template from storage: {error message}"`.
        *   `[ ]` 117.c.viii. Decode the downloaded content: if the result is an `ArrayBuffer`, use `new TextDecoder().decode(data)`, if it's a `Blob`, use `await data.text()`, otherwise throw an error: `"Invalid template file format"`.
        *   `[ ]` 117.c.ix. Use the decoded template content as `documentTemplateContent` instead of downloading from `docInfo.template_filename`.
        *   `[ ]` 117.c.x. If `prompt_text` is present (not null/empty), use it directly as `documentTemplateContent` (for backward compatibility with prompts that still have inline text).
        *   `[ ]` 117.c.xi. If both `prompt_text` is null/empty AND `document_template_id` is null/missing, throw an error: `"System prompt template is missing both prompt_text and document_template_id"`.
        *   `[ ]` 117.c.xii. Remove the code that gets `template_filename` from `headerContext.files_to_generate` and downloads it directly (lines 112-140), as this violates the canonical process.
    *   `[ ]` 117.d. `[TEST-UNIT]` Re-run the test from step 117.b and ensure it now passes, proving `assembleTurnPrompt` correctly follows the canonical template resolution process using `document_template_id`.
    *   `[ ]` 117.e. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-assembler/assembleTurnPrompt.test.ts`, add a new test case that:
        *   `[ ]` 117.e.i. Mocks a `stage` object with a `recipe_step` containing `prompt_template_id: "prompt-id-789"`.
        *   `[ ]` 117.e.ii. Mocks a database query to `system_prompts` that returns `{ prompt_text: "inline turn template content", document_template_id: null }` (non-null, non-empty string).
        *   `[ ]` 117.e.iii. Invokes `assembleTurnPrompt` with the mocked dependencies.
        *   `[ ]` 117.e.iv. Asserts that `downloadFromStorage` was NOT called for template download (proving backward compatibility with inline `prompt_text`).
        *   `[ ]` 117.e.v. Asserts that `renderPrompt` was called with `"inline turn template content"` as the template.
    *   `[ ]` 117.f. `[CRITERIA]` The implementation is complete when: (1) all tests in `assembleTurnPrompt.test.ts` pass, (2) the function follows the canonical template resolution process (queries `system_prompts` using `prompt_template_id`, gets `document_template_id`, queries `dialectic_document_templates`, downloads from canonical storage path), (3) the function maintains backward compatibility with inline `prompt_text`, (4) the function throws appropriate errors when templates are missing or downloads fail, (5) the function no longer uses `headerContext.files_to_generate[].template_filename` for template resolution, (6) the linter reports no errors or warnings.
*   `[ ]` 118. **`[COMMIT]` fix(BE): Update prompt assemblers to follow canonical template resolution using document_template_id**
    *   `[ ]` 118.a. Create a commit with the message "fix(BE): Update prompt assemblers to follow canonical template resolution using document_template_id" containing all the changes from steps 116-117. The commit body should explain that: (1) `assemblePlannerPrompt` now queries `document_template_id` from `system_prompts` when `prompt_text` is null, following the canonical process, (2) `assembleTurnPrompt` was updated to follow the canonical template resolution process instead of getting template filenames from `headerContext.files_to_generate[].template_filename`, which violated code standards, (3) both functions now: query `system_prompts` using `prompt_template_id`, get `document_template_id`, query `dialectic_document_templates` to get storage location (`storage_bucket`, `storage_path`, `file_name`), download the template file from storage using the canonical database-driven path, decode and use the downloaded content as the template, (4) backward compatibility is maintained for prompts that still have inline `prompt_text`, (5) this ensures all prompt assemblers follow the canonical database-driven template resolution process, enabling the intended architecture where `prompt_text` is NULL and templates are resolved via `document_template_id`. 