...
*   `[✅]` 108. **`[TEST-UNIT]` `supabase/functions/dialectic-service/generateContribution.test.ts` — Prove parent PLAN job includes all required fields in payload**
    *   `[✅]` 108.a. `[DEPS]` Parent PLAN jobs created by `generateContributions` must include all fields that downstream processors require. Analysis shows two critical blockers: (1) `assemblePlannerPrompt` requires `model_slug` (line 50-51, 140) for file upload path construction, and (2) `processSimpleJob` requires `user_jwt` (line 94-96) before routing to processors. Currently, parent PLAN jobs include `model_id` and `user_jwt` but not `model_slug`, causing planner child jobs to fail. The parent job must fetch the model name from the `ai_providers` table and include `model_slug: model.name` in the payload so all planners can inherit it along with all other parent fields. **CRITICAL PRINCIPLE:** All planners must inherit ALL fields from the parent job payload (including `model_slug`, `user_jwt`, and any other present fields) to avoid edge cases where downstream processors require fields not explicitly identified, following defensive programming principles.
    *   `[✅]` 108.b. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-service/generateContribution.test.ts`, add a new failing test case that:
        *   `[✅]` 108.b.i. Mocks a Supabase client with a query builder that returns a model from `ai_providers` with `name: "test-model-slug"` for a given `model_id`.
        *   `[✅]` 108.b.ii. Invokes `generateContributions` with a payload containing `selectedModelIds: ["model-123"]` and verifies the `authToken` is passed through.
        *   `[✅]` 108.b.iii. Asserts that the job payload inserted includes `model_slug: "test-model-slug"` (in addition to existing `user_jwt` and other fields).
        *   `[✅]` 108.b.iv. This test must fail because the current implementation does not fetch or include `model_slug`, proving the flaw.
    *   `[✅]` 108.c. `[BE]` **GREEN**: In `supabase/functions/dialectic-service/generateContribution.ts`, update the job creation loop (around lines 124-167) to:
        *   `[✅]` 108.c.i. For each `modelId` in `selectedModelIds`, query `dbClient.from("ai_providers").select("name").eq("id", modelId).single()` to fetch the model record.
        *   `[✅]` 108.c.ii. Extract the `name` field from the model record.
        *   `[✅]` 108.c.iii. Add `model_slug: model.name` to the `jobPayload` object before inserting the job (around line 127-133), ensuring `user_jwt: authToken` and all other existing fields remain intact.
        *   `[✅]` 108.c.iv. Handle database errors appropriately, ensuring the operation fails gracefully if the model is not found.
    *   `[✅]` 108.d. `[TEST-UNIT]` **GREEN**: Re-run the test from step 108.b and ensure it now passes, proving `generateContributions` correctly includes `model_slug` in parent PLAN job payloads alongside all other required fields.
    *   `[✅]` 108.e. `[LINT]` Run the linter for `supabase/functions/dialectic-service/generateContribution.test.ts` and `supabase/functions/dialectic-service/generateContribution.ts`, resolving any warnings or errors.
*   `[✅]` 109. **`[TEST-UNIT]` `supabase/functions/dialectic-worker/strategies/planners/planAllToOne.test.ts` — Prove planner inherits all fields from parent job**
    *   `[✅]` 109.a. `[DEPS]` The `planAllToOne` planner creates child EXECUTE job payloads that inherit fields from the parent PLAN job. After step 108, parent jobs include `model_slug` and `user_jwt`, but `planAllToOne` currently inherits only a subset of parent fields and omits critical ones. Child jobs routed to `assemblePlannerPrompt` require `model_slug` (line 50-51, 140), and `processSimpleJob` requires `user_jwt` (line 94-96). **CRITICAL PRINCIPLE:** All planners must inherit ALL fields from `parentJob.payload` (including `model_slug`, `user_jwt`, and any other present fields) to ensure child jobs have complete context for downstream processors, following defensive programming to avoid edge cases.
    *   `[✅]` 109.b. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-worker/strategies/planners/planAllToOne.test.ts`, add a new failing test case that:
        *   `[✅]` 109.b.i. Constructs a mock parent job payload with `model_slug: "parent-model-slug"`, `user_jwt: "parent-jwt-token"`, and all required properties.
        *   `[✅]` 109.b.ii. Invokes `planAllToOne` with source documents, parent job containing the mock payload, and a mock recipe step.
        *   `[✅]` 109.b.iii. Asserts that the returned payload array includes a job with `model_slug: "parent-model-slug"` and `user_jwt: "parent-jwt-token"`.
        *   `[✅]` 109.b.iv. This test must fail because the current implementation does not inherit `model_slug` or `user_jwt`, proving the flaw.
    *   `[✅]` 109.c. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/strategies/planners/planAllToOne.ts`, update the `newPayload` object construction (line 44-62) to inherit ALL fields from `parentJob.payload`, specifically including `model_slug: parentJob.payload.model_slug` and `user_jwt: parentJob.payload.user_jwt`, and any other present fields. Ensure child jobs have complete context from the parent.
    *   `[✅]` 109.d. `[TEST-UNIT]` **GREEN**: Re-run the test from step 109.b and ensure it now passes, proving `planAllToOne` correctly inherits all required fields in child payloads.
    *   `[✅]` 109.e. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/strategies/planners/planAllToOne.test.ts` and `supabase/functions/dialectic-worker/strategies/planners/planAllToOne.ts`, resolving any warnings or errors.
*   `[✅]` 110. **`[TEST-UNIT]` `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocument.test.ts` — Prove planner inherits all fields from parent job**
    *   `[✅]` 110.a. `[DEPS]` The `planPerSourceDocument` planner creates child EXECUTE job payloads that inherit fields from the parent PLAN job. After step 108, parent jobs include `model_slug` and `user_jwt`. While `planPerSourceDocument` currently inherits `user_jwt` (line 109), it does not inherit `model_slug`. Child jobs routed to `assemblePlannerPrompt` require `model_slug` (line 50-51, 140). **CRITICAL PRINCIPLE:** All planners must inherit ALL fields from `parentJob.payload` (including `model_slug`, `user_jwt`, and any other present fields) to ensure child jobs have complete context for downstream processors, following defensive programming.
    *   `[✅]` 110.b. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocument.test.ts`, add a new failing test case that:
        *   `[✅]` 110.b.i. Constructs a mock parent job payload with `model_slug: "parent-model-slug"`, `user_jwt: "parent-jwt-token"`, and all required properties.
        *   `[✅]` 110.b.ii. Invokes `planPerSourceDocument` with source documents, parent job containing the mock payload, and a mock recipe step.
        *   `[✅]` 110.b.iii. Asserts that every job in the returned payload array includes `model_slug: "parent-model-slug"` (in addition to the existing `user_jwt`).
        *   `[✅]` 110.b.iv. This test must fail because the current implementation does not inherit `model_slug`, proving the flaw.
    *   `[✅]` 110.c. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocument.ts`, update the `newPayload` object construction (line 94-112) to inherit ALL fields from `parentJob.payload`, specifically adding `model_slug: parentJob.payload.model_slug` (in addition to the existing `user_jwt` inheritance), and ensure all other present fields are inherited. Ensure child jobs have complete context from the parent.
    *   `[✅]` 110.d. `[TEST-UNIT]` **GREEN**: Re-run the test from step 110.b and ensure it now passes, proving `planPerSourceDocument` correctly inherits all required fields in all child payloads.
    *   `[✅]` 110.e. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocument.test.ts` and `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocument.ts`, resolving any warnings or errors.
*   `[✅]` 111. **`[TEST-UNIT]` `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocumentByLineage.test.ts` — Prove planner inherits all fields from parent job**
    *   `[✅]` 111.a. `[DEPS]` The `planPerSourceDocumentByLineage` planner creates child EXECUTE job payloads that inherit fields from the parent PLAN job. After step 108, parent jobs include `model_slug` and `user_jwt`, but `planPerSourceDocumentByLineage` currently inherits only a subset of parent fields and omits critical ones. Child jobs routed to `assemblePlannerPrompt` require `model_slug` (line 50-51, 140), and `processSimpleJob` requires `user_jwt` (line 94-96). **CRITICAL PRINCIPLE:** All planners must inherit ALL fields from `parentJob.payload` (including `model_slug`, `user_jwt`, and any other present fields) to ensure child jobs have complete context for downstream processors, following defensive programming to avoid edge cases.
    *   `[✅]` 111.b. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocumentByLineage.test.ts`, add a new failing test case that:
        *   `[✅]` 111.b.i. Constructs a mock parent job payload with `model_slug: "parent-model-slug"`, `user_jwt: "parent-jwt-token"`, and all required properties.
        *   `[✅]` 111.b.ii. Invokes `planPerSourceDocumentByLineage` with source documents, parent job containing the mock payload, and a mock recipe step.
        *   `[✅]` 111.b.iii. Asserts that every job in the returned payload array includes `model_slug: "parent-model-slug"` and `user_jwt: "parent-jwt-token"`.
        *   `[✅]` 111.b.iv. This test must fail because the current implementation does not inherit `model_slug` or `user_jwt`, proving the flaw.
    *   `[✅]` 111.c. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocumentByLineage.ts`, update the `newPayload` object construction (line 69-90) to inherit ALL fields from `parentJob.payload`, specifically including `model_slug: parentJob.payload.model_slug` and `user_jwt: parentJob.payload.user_jwt`, and any other present fields. Ensure child jobs have complete context from the parent.
    *   `[✅]` 111.d. `[TEST-UNIT]` **GREEN**: Re-run the test from step 111.b and ensure it now passes, proving `planPerSourceDocumentByLineage` correctly inherits all required fields in all child payloads.
    *   `[✅]` 111.e. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocumentByLineage.test.ts` and `supabase/functions/dialectic-worker/strategies/planners/planPerSourceDocumentByLineage.ts`, resolving any warnings or errors.
*   `[✅]` 112. **`[TEST-UNIT]` `supabase/functions/dialectic-worker/strategies/planners/planPerSourceGroup.test.ts` — Prove planner inherits all fields from parent job**
    *   `[✅]` 112.a. `[DEPS]` The `planPerSourceGroup` planner creates child EXECUTE job payloads that inherit fields from the parent PLAN job. After step 108, parent jobs include `model_slug` and `user_jwt`, but `planPerSourceGroup` currently inherits only a subset of parent fields and omits critical ones. Child jobs routed to `assemblePlannerPrompt` require `model_slug` (line 50-51, 140), and `processSimpleJob` requires `user_jwt` (line 94-96). **CRITICAL PRINCIPLE:** All planners must inherit ALL fields from `parentJob.payload` (including `model_slug`, `user_jwt`, and any other present fields) to ensure child jobs have complete context for downstream processors, following defensive programming to avoid edge cases.
    *   `[✅]` 112.b. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-worker/strategies/planners/planPerSourceGroup.test.ts`, add a new failing test case that:
        *   `[✅]` 112.b.i. Constructs a mock parent job payload with `model_slug: "parent-model-slug"`, `user_jwt: "parent-jwt-token"`, and all required properties.
        *   `[✅]` 112.b.ii. Invokes `planPerSourceGroup` with source documents, parent job containing the mock payload, and a mock recipe step.
        *   `[✅]` 112.b.iii. Asserts that every job in the returned payload array includes `model_slug: "parent-model-slug"` and `user_jwt: "parent-jwt-token"`.
        *   `[✅]` 112.b.iv. This test must fail because the current implementation does not inherit `model_slug` or `user_jwt`, proving the flaw.
    *   `[✅]` 112.c. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/strategies/planners/planPerSourceGroup.ts`, update the `newPayload` object construction (line 54-75) to inherit ALL fields from `parentJob.payload`, specifically including `model_slug: parentJob.payload.model_slug` and `user_jwt: parentJob.payload.user_jwt`, and any other present fields. Ensure child jobs have complete context from the parent.
    *   `[✅]` 112.d. `[TEST-UNIT]` **GREEN**: Re-run the test from step 112.b and ensure it now passes, proving `planPerSourceGroup` correctly inherits all required fields in all child payloads.
    *   `[✅]` 112.e. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/strategies/planners/planPerSourceGroup.test.ts` and `supabase/functions/dialectic-worker/strategies/planners/planPerSourceGroup.ts`, resolving any warnings or errors.
*   `[✅]` 113. **`[TEST-UNIT]` `supabase/functions/dialectic-worker/strategies/planners/planPairwiseByOrigin.test.ts` — Prove planner inherits all fields from parent job**
    *   `[✅]` 113.a. `[DEPS]` The `planPairwiseByOrigin` planner creates child EXECUTE job payloads that inherit fields from the parent PLAN job. After step 108, parent jobs include `model_slug` and `user_jwt`, but `planPairwiseByOrigin` currently inherits only a subset of parent fields and omits critical ones. Child jobs routed to `assemblePlannerPrompt` require `model_slug` (line 50-51, 140), and `processSimpleJob` requires `user_jwt` (line 94-96). **CRITICAL PRINCIPLE:** All planners must inherit ALL fields from `parentJob.payload` (including `model_slug`, `user_jwt`, and any other present fields) to ensure child jobs have complete context for downstream processors, following defensive programming to avoid edge cases.
    *   `[✅]` 113.b. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-worker/strategies/planners/planPairwiseByOrigin.test.ts`, add a new failing test case that:
        *   `[✅]` 113.b.i. Constructs a mock parent job payload with `model_slug: "parent-model-slug"`, `user_jwt: "parent-jwt-token"`, and all required properties.
        *   `[✅]` 113.b.ii. Invokes `planPairwiseByOrigin` with source documents, parent job containing the mock payload, and a mock recipe step.
        *   `[✅]` 113.b.iii. Asserts that every job in the returned payload array includes `model_slug: "parent-model-slug"` and `user_jwt: "parent-jwt-token"`.
        *   `[✅]` 113.b.iv. This test must fail because the current implementation does not inherit `model_slug` or `user_jwt`, proving the flaw.
    *   `[✅]` 113.c. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/strategies/planners/planPairwiseByOrigin.ts`, update the `newPayload` object construction (line 91-112) to inherit ALL fields from `parentJob.payload`, specifically including `model_slug: parentJob.payload.model_slug` and `user_jwt: parentJob.payload.user_jwt`, and any other present fields. Ensure child jobs have complete context from the parent.
    *   `[✅]` 113.d. `[TEST-UNIT]` **GREEN**: Re-run the test from step 113.b and ensure it now passes, proving `planPairwiseByOrigin` correctly inherits all required fields in all child payloads.
    *   `[✅]` 113.e. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/strategies/planners/planPairwiseByOrigin.test.ts` and `supabase/functions/dialectic-worker/strategies/planners/planPairwiseByOrigin.ts`, resolving any warnings or errors.
*   `[✅]` 114. **`[TEST-UNIT]` `supabase/functions/dialectic-worker/strategies/planners/planPerModel.test.ts` — Prove planner inherits all fields from parent job**
    *   `[✅]` 114.a. `[DEPS]` The `planPerModel` planner creates child EXECUTE job payloads that inherit fields from the parent PLAN job. After step 108, parent jobs include `model_slug` and `user_jwt`, but `planPerModel` currently inherits only a subset of parent fields and omits critical ones. Child jobs routed to `assemblePlannerPrompt` require `model_slug` (line 50-51, 140), and `processSimpleJob` requires `user_jwt` (line 94-96). **CRITICAL PRINCIPLE:** All planners must inherit ALL fields from `parentJob.payload` (including `model_slug`, `user_jwt`, and any other present fields) to ensure child jobs have complete context for downstream processors, following defensive programming to avoid edge cases.
    *   `[✅]` 114.b. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-worker/strategies/planners/planPerModel.test.ts`, add a new failing test case that:
        *   `[✅]` 114.b.i. Constructs a mock parent job payload with `model_slug: "parent-model-slug"`, `user_jwt: "parent-jwt-token"`, and all required properties.
        *   `[✅]` 114.b.ii. Invokes `planPerModel` with source documents, parent job containing the mock payload, and a mock recipe step.
        *   `[✅]` 114.b.iii. Asserts that every job in the returned payload array includes `model_slug: "parent-model-slug"` and `user_jwt: "parent-jwt-token"`.
        *   `[✅]` 114.b.iv. This test must fail because the current implementation does not inherit `model_slug` or `user_jwt`, proving the flaw.
    *   `[✅]` 114.c. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/strategies/planners/planPerModel.ts`, update the `newPayload` object construction (line 75-93) to inherit ALL fields from `parentJob.payload`, specifically including `model_slug: parentJob.payload.model_slug` and `user_jwt: parentJob.payload.user_jwt`, and any other present fields. Ensure child jobs have complete context from the parent.
    *   `[✅]` 114.d. `[TEST-UNIT]` **GREEN**: Re-run the test from step 114.b and ensure it now passes, proving `planPerModel` correctly inherits all required fields in all child payloads.
    *   `[✅]` 114.e. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/strategies/planners/planPerModel.test.ts` and `supabase/functions/dialectic-worker/strategies/planners/planPerModel.ts`, resolving any warnings or errors.
*   `[✅]` 115. **`[COMMIT]` fix(BE): Add model_slug to parent PLAN jobs and inherit all fields in all planner child jobs**
    *   `[✅]` 115.a. Create a commit with the message "fix(BE): Add model_slug to parent PLAN jobs and inherit all fields in all planner child jobs" containing all the changes from steps 108-114. The commit body should explain that: (1) parent PLAN jobs now fetch and include `model_slug` from the `ai_providers` table, (2) all 6 planners now inherit ALL fields from `parentJob.payload` (including `model_slug`, `user_jwt`, and any other present fields) in child EXECUTE job payloads, following defensive programming principles to ensure complete context for downstream processors. This enables successful routing to `assemblePlannerPrompt` (which requires `model_slug` for file upload path construction) and `processSimpleJob` (which requires `user_jwt` before routing), preventing edge cases where missing fields cause failures.
*   `[✅]` 116. **`[BE]` Update `assemblePlannerPrompt` to fetch templates from storage using `document_template_id`**
    *   `[✅]` 116.a. `[DEPS]` The `assemblePlannerPrompt` function in `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.ts` currently queries `system_prompts` for `prompt_text` directly (line 70) and uses it as the template (line 106). However, migrations set `prompt_text` to path references like `\path=docs/prompts/thesis/thesis_planner_header_v1.md` instead of NULL, preventing the intended fallback. The function depends on: `dbClient` (SupabaseClient) for database queries, `downloadFromStorage` (from `supabase_storage_utils.ts`) for file downloads, and `render` function for prompt rendering. The correct architecture requires: (1) query `system_prompts` for both `prompt_text` and `document_template_id`, (2) if `prompt_text` is null/empty and `document_template_id` exists, query `dialectic_document_templates` to get `storage_bucket`, `storage_path`, `file_name`, (3) download template from storage using `downloadFromStorage(dbClient, storage_bucket, fullPath)`, (4) decode the downloaded content and use it as the template. This ensures templates are fetched from storage rather than embedded in the database or passed as path strings.
    *   `[✅]` 116.b. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.test.ts`, add a new failing test case that:
        *   `[✅]` 116.b.i. Mocks a `system_prompts` record with `prompt_text: null` and `document_template_id: "template-uuid-123"`.
        *   `[✅]` 116.b.ii. Mocks a `dialectic_document_templates` record with `id: "template-uuid-123"`, `storage_bucket: "prompt-templates"`, `storage_path: "docs/prompts/thesis/"`, `file_name: "thesis_planner_header_v1.md"`.
        *   `[✅ ]` 116.b.iii. Mocks `downloadFromStorage` to return template content as `ArrayBuffer` containing `"# Template Content\n\nThis is the actual template."`.
        *   `[✅]` 116.b.iv. Invokes `assemblePlannerPrompt` with the mocked dependencies.
        *   `[✅]` 116.b.v. Asserts that `downloadFromStorage` was called with `dbClient`, `"prompt-templates"`, and `"docs/prompts/thesis/thesis_planner_header_v1.md"`.
        *   `[✅]` 116.b.vi. Asserts that the `render` function was called with a `stageWithOverride` containing `system_prompts: { prompt_text: "# Template Content\n\nThis is the actual template." }` (the downloaded content, not a path string or null).
        *   `[✅]` 116.b.vii. This test must fail because the current implementation queries only `prompt_text` and does not fetch from `document_template_id`, proving the flaw.
    *   `[✅]` 116.c. `[BE]` In `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.ts`, update the template fetching logic (lines 68-106) to:
        *   `[✅]` 116.c.i. Change the query from `.select("prompt_text")` to `.select("prompt_text, document_template_id")` to fetch both fields.
        *   `[✅]` 116.c.ii. After fetching `promptTemplateData`, check if `promptTemplateData.prompt_text` is null or empty. If it is null/empty AND `promptTemplateData.document_template_id` exists and is a non-empty string, proceed with template resolution from storage.
        *   `[✅]` 116.c.iii. If `document_template_id` is present, query `dialectic_document_templates` table using `.from("dialectic_document_templates").select("storage_bucket, storage_path, file_name").eq("id", promptTemplateData.document_template_id).single()` to get the template storage location.
        *   `[✅]` 116.c.iv. Validate that `storage_bucket`, `storage_path`, and `file_name` are all present and non-empty strings. If any are missing, throw an error: `"Invalid template record: missing storage_bucket, storage_path, or file_name"`.
        *   `[✅]` 116.c.v. Construct the full storage path by combining `storage_path` and `file_name`, ensuring no double slashes (remove trailing slash from `storage_path` if present, ensure `file_name` doesn't start with a slash): `const fullPath = \`${storage_path.replace(/\/$/, '')}/${file_name}\`;`.
        *   `[✅]` 116.c.vi. Call `downloadFromStorage(dbClient, storage_bucket, fullPath)` to download the template file.
        *   `[✅]` 116.c.vii. If the download fails (`error` is not null) or returns no data (`data` is null), throw an error: `"Failed to download template from storage: {error message}"`.
        *   `[✅]` 116.c.viii. Decode the downloaded content: if the result is an `ArrayBuffer`, use `new TextDecoder().decode(data)`, if it's a `Blob`, use `await data.text()`, otherwise throw an error: `"Invalid template file format"`.
        *   `[✅]` 116.c.ix. Use the decoded template content as `promptTemplate` instead of `promptTemplateData.prompt_text`.
        *   `[✅]` 116.c.x. If `prompt_text` is present (not null/empty), use it directly as before (for backward compatibility with prompts that still have inline text).
        *   `[✅]` 116.c.xi. If both `prompt_text` is null/empty AND `document_template_id` is null/missing, throw an error: `"System prompt template is missing both prompt_text and document_template_id"`.
    *   `[✅]` 116.d. `[TEST-UNIT]` Re-run the test from step 116.b and ensure it now passes, proving `assemblePlannerPrompt` correctly fetches templates from storage using `document_template_id`.
    *   `[✅]` 116.e. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.test.ts`, add a new test case that:
        *   `[✅]` 116.e.i. Mocks a `system_prompts` record with `prompt_text: "inline template content"` (non-null, non-empty string).
        *   `[✅]` 116.e.ii. Invokes `assemblePlannerPrompt` with the mocked dependencies.
        *   `[✅]` 116.e.iii. Asserts that `downloadFromStorage` was NOT called (proving backward compatibility with inline `prompt_text`).
        *   `[✅]` 116.e.iv. Asserts that the `render` function was called with a `stageWithOverride` containing `system_prompts: { prompt_text: "inline template content" }`.
    *   `[✅]` 116.f. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.test.ts`, add a new test case that:
        *   `[✅]` 116.f.i. Mocks a `system_prompts` record with `prompt_text: null` and `document_template_id: null`.
        *   `[✅]` 116.f.ii. Invokes `assemblePlannerPrompt` with the mocked dependencies.
        *   `[✅]` 116.f.iii. Asserts that the function throws an error with message containing `"System prompt template is missing both prompt_text and document_template_id"`.
    *   `[✅]` 116.g. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.test.ts`, add a new test case that:
        *   `[✅]` 116.g.i. Mocks a `system_prompts` record with `prompt_text: null` and `document_template_id: "template-uuid-123"`.
        *   `[✅]` 116.g.ii. Mocks a `dialectic_document_templates` record with `id: "template-uuid-123"`, `storage_bucket: "prompt-templates"`, `storage_path: "docs/prompts/thesis/"`, `file_name: "thesis_planner_header_v1.md"`.
        *   `[✅]` 116.g.iii. Mocks `downloadFromStorage` to return an error: `{ data: null, error: new Error("File not found") }`.
        *   `[✅]` 116.g.iv. Invokes `assemblePlannerPrompt` with the mocked dependencies.
        *   `[✅]` 116.g.v. Asserts that the function throws an error with message containing `"Failed to download template from storage"`.
    *   `[✅]` 116.h. `[CRITERIA]` The implementation is complete when: (1) all tests in `assemblePlannerPrompt.test.ts` pass, (2) the function correctly fetches templates from storage when `prompt_text` is null and `document_template_id` is present, (3) the function maintains backward compatibility with inline `prompt_text`, (4) the function throws appropriate errors when templates are missing or downloads fail, (5) the linter reports no errors or warnings.
*   `[✅]` 117. **`[BE]` Update `renderPrompt` to support both single and double brace placeholders**
    *   `[✅]` 117.a. `[DEPS]` The `renderPrompt` function in `supabase/functions/_shared/prompt-renderer.ts` is architecturally flawed. It only matches single-brace placeholders `{key}` (line 71), but templates use three different brace patterns: (1) double braces `{{#section:key}}...{{/section:key}}` for conditional sections (already handled correctly on line 54), (2) double braces `{{key}}` for content placeholders outside conditional sections (used in `thesis_planner_header_v1.md`, `thesis_business_case_turn_v1.md`), and (3) single braces `{key}` for content placeholders inside conditional sections (used in `dialectic_seed_prompt_v1.md`). When processing `{{role}}`, the regex matches only the inner `{role}`, replaces it with the value, and leaves the outer braces, resulting in `{senior product strategist and technical architect}` instead of `senior product strategist and technical architect`. The documentation (line 20) says variables are expected in `{{variable_name}}` format, but the implementation only handles `{variable_name}`. The function depends on: `escapeRegExp` helper for regex escaping, `mergedVariables` object for substitution values, `renderedText` string for processing, and conditional section handling that runs before variable substitution. The correct architecture requires: (1) match double braces `{{key}}` first (after conditional sections are processed), (2) then match single braces `{key}` for backward compatibility, (3) update line removal logic (line 75) to match both formats, (4) update final cleanup regex (line 97) to remove both single and double brace placeholders. This ensures both `{key}` and `{{key}}` are properly substituted without leaving extra braces in the output.
    *   `[✅]` 117.b. `[TEST-UNIT]` **RED**: In `supabase/functions/_shared/prompt-renderer.test.ts`, add new failing test cases that prove the existing implementation incorrectly handles double-brace placeholders:
        *   `[✅]` 117.b.i. Test case: "should substitute double-brace placeholders without leaving extra braces". The test must:
            *   `[✅]` 117.b.i.1. Provide a `basePromptText` containing `"You are a {{role}}, act accordingly."` with double braces.
            *   `[✅]` 117.b.i.2. Provide a `renderContext` containing `role: "senior product strategist and technical architect"`.
            *   `[✅]` 117.b.i.3. Assert the `expectedOutput` is `"You are a senior product strategist and technical architect, act accordingly."` (no braces around the value).
            *   `[✅]` 117.b.i.4. This test must fail because the current implementation produces `"You are a {senior product strategist and technical architect}, act accordingly."` (with extra braces), proving the flaw.
        *   `[✅]` 117.b.ii. Test case: "should support both single and double brace placeholders in the same template". The test must:
            *   `[✅]` 117.b.ii.1. Provide a `basePromptText` containing both `{{original_user_request}}` (double braces) and `{context_description}` (single braces inside a conditional section).
            *   `[✅]` 117.b.ii.2. Provide a `renderContext` containing both `original_user_request: "User wants a notepad app"` and `context_description: "I want to create a notepad app"`.
            *   `[✅]` 117.b.ii.3. Assert the `expectedOutput` correctly substitutes both placeholders without extra braces.
            *   `[✅]` 117.b.ii.4. This test must fail because the current implementation leaves braces around double-brace substitutions, proving the flaw.
        *   `[✅]` 117.b.iii. Test case: "should remove lines containing unknown double-brace placeholders". The test must:
            *   `[✅]` 117.b.iii.1. Provide a `basePromptText` containing `"- **User Objective**: {{unknown_var}}\n- **Stage Role**: {{role}}"` with one unknown double-brace placeholder.
            *   `[✅]` 117.b.iii.2. Provide a `renderContext` containing `role: "architect"` but no `unknown_var`.
            *   `[✅]` 117.b.iii.3. Assert the `expectedOutput` removes the entire line containing `{{unknown_var}}` but keeps the line with `{{role}}` substituted correctly.
            *   `[✅]` 117.b.iii.4. This test must fail because the current implementation does not remove lines with unknown double-brace placeholders, proving the flaw.
        *   `[✅]` 117.b.iv. Test case: "should remove unknown double-brace placeholders in final cleanup". The test must:
            *   `[✅]` 117.b.iv.1. Provide a `basePromptText` containing `"Known: {{role}}. Unknown: {{missing_var}}."` with one known and one unknown double-brace placeholder.
            *   `[✅]` 117.b.iv.2. Provide a `renderContext` containing `role: "architect"` but no `missing_var`.
            *   `[✅]` 117.b.iv.3. Assert the `expectedOutput` is `"Known: architect. Unknown: ."` (or removes the unknown placeholder entirely, depending on the chosen behavior).
            *   `[✅]` 117.b.iv.4. This test must fail because the current implementation leaves `{{missing_var}}` in the output or does not clean it up properly, proving the flaw.
    *   `[✅]` 117.c. `[BE]` **GREEN**: In `supabase/functions/_shared/prompt-renderer.ts`, update the variable substitution logic (lines 66-89) to support both single and double brace placeholders:
        *   `[✅]` 117.c.i. Update the variable substitution loop (line 67) to process placeholders in two passes: first double braces, then single braces, to avoid conflicts when both exist.
        *   `[✅]` 117.c.ii. **First pass (double braces)**: For each key in `mergedVariables`, create a regex that matches double braces: `new RegExp(`\{\{\s*${escapedKey}\s*\}\}`, 'g')` (matching `{{key}}` with optional whitespace).
        *   `[✅]` 117.c.iii. **Second pass (single braces)**: For each key in `mergedVariables`, create a regex that matches single braces: `new RegExp(`{\\s*${escapedKey}\\s*}`, 'g')` (matching `{key}` with optional whitespace, existing pattern).
        *   `[✅]` 117.c.iv. For each pass, if the value is null/undefined/empty, update the line removal regex (line 75) to match both formats: `new RegExp(`^.*\{\{\s*${escapedKey}\s*\}\}.*$\\n?`, "gm")` for double braces and `new RegExp(`^.*{\\s*${escapedKey}\\s*}.*$\\n?`, "gm")` for single braces.
        *   `[✅]` 117.c.v. For each pass, if the value exists, perform the substitution using `renderedText.replace(placeholderRegex, stringValue)` as before.
        *   `[✅]` 117.c.vi. Update the final cleanup regex (line 97) to remove both single and double brace placeholders: `renderedText.replace(/^.*\{\{[A-Za-z0-9_]+\}\}.*$\n?/gm, '').replace(/^.*\{[A-Za-z0-9_]+\}.*$\n?/gm, '')` (removing unknown double-brace placeholders first, then unknown single-brace placeholders).
        *   `[✅]` 117.c.vii. Ensure conditional sections (line 54) continue to work correctly and are processed before variable substitution as before.
        *   `[✅]` 117.c.viii. Ensure backward compatibility: all existing single-brace templates continue to work without modification.
    *   `[✅]` 117.d. `[TEST-UNIT]` **GREEN**: Re-run all tests from step 117.b and ensure they now pass, proving `renderPrompt` correctly handles both single and double brace placeholders without leaving extra braces in the output.
    *   `[✅]` 117.e. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-renderer.test.ts`, add a new test case that proves backward compatibility with existing single-brace templates:
        *   `[✅]` 117.e.i. Test case: "should maintain backward compatibility with single-brace placeholders". The test must:
            *   `[✅]` 117.e.i.1. Provide a `basePromptText` containing `"Hello {name}! Welcome to {place}."` with single braces (existing pattern).
            *   `[✅]` 117.e.i.2. Provide a `renderContext` containing `name: "World"` and `place: "Earth"`.
            *   `[✅]` 117.e.i.3. Assert the `expectedOutput` is `"Hello World! Welcome to Earth."` (proving single braces still work).
            *   `[✅]` 117.e.i.4. This test must pass, proving backward compatibility is maintained.
    *   `[✅]` 117.f. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-renderer.test.ts`, add a new test case that proves mixed brace styles work correctly:
        *   `[✅]` 117.f.i. Test case: "should handle mixed single and double brace placeholders correctly". The test must:
            *   `[✅]` 117.f.i.1. Provide a `basePromptText` containing `"Double: {{role}}. Single: {stage_instructions}. Both: {{original_user_request}} and {domain}."` with both brace styles.
            *   `[✅]` 117.f.i.2. Provide a `renderContext` containing `role: "architect"`, `stage_instructions: "establish baseline"`, `original_user_request: "Create app"`, and `domain: "Software Development"`.
            *   `[✅]` 117.f.i.3. Assert the `expectedOutput` correctly substitutes all four placeholders without extra braces: `"Double: architect. Single: establish baseline. Both: Create app and Software Development."`.
            *   `[✅]` 117.f.i.4. This test must pass, proving both formats work together correctly.
    *   `[✅]` 117.g. `[TEST-UNIT]` In `supabase/functions/_shared/prompt-renderer.test.ts`, verify that the existing test suite (all tests from lines 50-203) continues to pass after the implementation changes, proving no regressions were introduced.
    *   `[✅]` 117.h. `[LINT]` Run the linter for `supabase/functions/_shared/prompt-renderer.ts` and `supabase/functions/_shared/prompt-renderer.test.ts`, resolving any warnings or errors introduced by the changes.
    *   `[✅]` 117.i. `[CRITERIA]` The implementation is complete when: (1) all tests in `prompt-renderer.test.ts` pass (including new double-brace tests and existing single-brace tests), (2) the function correctly substitutes both `{key}` and `{{key}}` placeholders without leaving extra braces, (3) the function removes lines containing unknown placeholders in both formats, (4) the function maintains backward compatibility with existing single-brace templates, (5) the final cleanup removes unknown placeholders in both formats, (6) the linter reports no errors or warnings.
*   `[✅]` 118. **`[TYPES]` `supabase/functions/_shared/prompt-assembler/prompt-assembler.interface.ts` — Add `projectInitialUserPrompt` to `AssemblePlannerPromptDeps` to unify code path**
    *   `[✅]` 118.a. `[DEPS]` The `assemblePlannerPrompt` function has two parallel and partially overlapping code paths for obtaining the initial user prompt: (1) `processSimpleJob.ts` resolves `projectInitialUserPrompt` from either `project.initial_user_prompt` or storage via `getInitialPromptContent`, passing it in `assembleOptions.projectInitialUserPrompt`, but (2) `assemblePlannerPrompt.ts` reads `project.initial_user_prompt` directly (lines 56-61) and passes it to `gatherContext`. The second path can be empty/null when the prompt is in storage, causing `original_user_request` to be empty and the User Objective section to be removed from rendered prompts. The `assembleSeedPrompt` function already uses the first path correctly via `AssembleSeedPromptDeps.projectInitialUserPrompt` (interface line 67). To unify the code paths, `AssemblePlannerPromptDeps` must include `projectInitialUserPrompt: string` (matching `AssembleSeedPromptDeps`), `prompt-assembler.ts` must pass `options.projectInitialUserPrompt` when calling `assemblePlannerPrompt`, and `assemblePlannerPrompt.ts` must use the parameter from deps instead of reading from `project.initial_user_prompt`. This requires updating the interface first (types file, exempt from tests), then updating the consumer (`prompt-assembler.ts`), then updating the implementation (`assemblePlannerPrompt.ts`).
    *   `[✅]` 118.b. `[TYPES]` In `supabase/functions/_shared/prompt-assembler/prompt-assembler.interface.ts`, add `projectInitialUserPrompt: string` to the `AssemblePlannerPromptDeps` interface (after `stage: StageContext`, before `gatherContext: GatherContextFn`) to match `AssembleSeedPromptDeps` and enable the unified code path. This ensures both prompt assemblers receive the resolved `projectInitialUserPrompt` from `processSimpleJob.ts` instead of reading from `project.initial_user_prompt` directly.
*   `[✅]` 119. **`[BE]` `supabase/functions/_shared/prompt-assembler/prompt-assembler.ts` — Pass `projectInitialUserPrompt` to `assemblePlannerPrompt`**
    *   `[✅]` 119.a. `[TEST-UNIT]` **RED**: In `supabase/functions/_shared/prompt-assembler/prompt-assembler.test.ts`, add a new failing test case that proves `assemblePlannerPrompt` is not called with `projectInitialUserPrompt` from `options`:
        *   `[✅]` 119.a.i. Test case: "should pass projectInitialUserPrompt from options to assemblePlannerPrompt". The test must:
            *   `[✅]` 119.a.i.1. Create an `AssemblePromptOptions` with `projectInitialUserPrompt: "resolved from storage"` (different from `project.initial_user_prompt`).
            *   `[✅]` 119.a.i.2. Create a mock `assemblePlannerPromptFn` that captures its `deps` argument.
            *   `[✅]` 119.a.i.3. Call `assembler.assemble(options)` with a job that has `job_type: "PLAN"`.
            *   `[✅]` 119.a.i.4. Assert the `assemblePlannerPromptFn` was called with `deps.projectInitialUserPrompt === "resolved from storage"`.
            *   `[✅]` 119.a.i.5. This test must fail because `prompt-assembler.ts` does not yet pass `options.projectInitialUserPrompt` to `assemblePlannerPrompt`, proving the flaw.
    *   `[✅]` 119.b. `[BE]` **GREEN**: In `supabase/functions/_shared/prompt-assembler/prompt-assembler.ts`, update the `assemblePlannerPrompt` call (lines 100-110) to include `projectInitialUserPrompt: options.projectInitialUserPrompt` in the deps object, ensuring the resolved `projectInitialUserPrompt` from `processSimpleJob.ts` is passed through to `assemblePlannerPrompt`.
    *   `[✅]` 119.c. `[TEST-UNIT]` **GREEN**: Re-run the test from step 119.a and ensure it passes, proving `prompt-assembler.ts` now correctly passes `projectInitialUserPrompt` from options to `assemblePlannerPrompt`.
    *   `[✅]` 119.d. `[LINT]` Run the linter for `supabase/functions/_shared/prompt-assembler/prompt-assembler.ts` and `supabase/functions/_shared/prompt-assembler/prompt-assembler.test.ts`, resolving any warnings or errors.
*   `[✅]` 120. **`[BE]` `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.ts` — Use `projectInitialUserPrompt` from deps instead of `project.initial_user_prompt`**
    *   `[✅]` 120.a. `[TEST-UNIT]` **RED**: In `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.test.ts`, update the existing test that verifies `gatherContext` is called with the correct `projectInitialUserPrompt` (around line 288) to prove the flaw:
        *   `[✅]` 120.a.i. Update the test setup to provide `projectInitialUserPrompt: "resolved prompt from storage"` in the deps (different from `defaultProject.initial_user_prompt`).
        *   `[✅]` 120.a.ii. Assert that `gatherContext` is called with `gatherArgs[6] === "resolved prompt from storage"` (the parameter from deps, not from `project.initial_user_prompt`).
        *   `[✅]` 120.a.iii. This test must fail because `assemblePlannerPrompt.ts` currently reads from `project.initial_user_prompt` instead of using `deps.projectInitialUserPrompt`, proving the flaw.
    *   `[✅]` 120.b. `[BE]` **GREEN**: In `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.ts`, update the function to use `projectInitialUserPrompt` from deps instead of reading from `project.initial_user_prompt`:
        *   `[✅]` 120.b.i. Remove the precondition check for `project.initial_user_prompt` (lines 56-60) since the value now comes from deps.
        *   `[✅]` 120.b.ii. Remove the local variable `initialUserPrompt` assignment from `project.initial_user_prompt` (line 61).
        *   `[✅]` 120.b.iii. Update the `gatherContext` call (line 209) to pass `projectInitialUserPrompt` from deps (`deps.projectInitialUserPrompt`) instead of the local `initialUserPrompt` variable.
        *   `[✅]` 120.b.iv. Ensure the function signature accepts `projectInitialUserPrompt` from `deps` (already added in step 118.b).
    *   `[✅]` 120.c. `[TEST-UNIT]` **GREEN**: Re-run the test from step 120.a and ensure it passes, proving `assemblePlannerPrompt.ts` now correctly uses `projectInitialUserPrompt` from deps and passes it to `gatherContext`.
    *   `[✅]` 120.d. `[TEST-UNIT]` Re-run all existing tests in `assemblePlannerPrompt.test.ts` to ensure no regressions were introduced by removing the `project.initial_user_prompt` check and switching to the deps parameter.
    *   `[✅]` 120.e. `[LINT]` Run the linter for `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.ts` and `supabase/functions/_shared/prompt-assembler/assemblePlannerPrompt.test.ts`, resolving any warnings or errors.
    *   `[✅]` 120.f. `[CRITERIA]` The implementation is complete when: (1) all tests in `assemblePlannerPrompt.test.ts` pass, (2) `assemblePlannerPrompt` uses `projectInitialUserPrompt` from deps instead of reading from `project.initial_user_prompt`, (3) `gatherContext` receives the resolved `projectInitialUserPrompt` value, (4) `original_user_request` in `DynamicContextVariables` is populated correctly, (5) the User Objective section is not removed from rendered prompts, (6) both `assembleSeedPrompt` and `assemblePlannerPrompt` use the same unified code path for obtaining the initial user prompt, (7) the linter reports no errors or warnings.
*   `[✅]` 121. **`[TYPES]` `supabase/functions/_shared/types/jsonSanitizer.interface.ts` — Define types for JSON sanitization utility**
    *   `[✅]` 121.a. `[DEPS]` The `executeModelCallAndSave` function in `supabase/functions/dialectic-worker/executeModelCallAndSave.ts` contains inline JSON parsing logic with immediate retry on failure (lines 933-957). AI responses often include common formatting deviations (single quotes, double quotes, triple backticks, markdown code blocks, leading/trailing whitespace) that wrap valid JSON content, causing unnecessary retries. The fix is to extract the JSON sanitization logic into a reusable utility function that handles common wrapper patterns before parsing, reducing retry costs while maintaining robustness for truly malformed JSON. The utility requires: (1) a type definition for the sanitization result (indicating whether sanitization occurred), (2) type guards to validate sanitized JSON content, (3) a core sanitization function that removes common wrapper patterns, and (4) integration into `executeModelCallAndSave` to replace inline parsing/retry logic.
    *   `[✅]` 121.b. `[TYPES]` In a new file `supabase/functions/_shared/types/jsonSanitizer.interface.ts`, define and export a new type `JsonSanitizationResult` that represents the result of sanitizing JSON content. This type must include: (1) `sanitized: string` - the sanitized JSON string content, (2) `wasSanitized: boolean` - a flag indicating whether any sanitization operations were performed (removal of wrappers, trimming, etc.), (3) `originalLength: number` - the original content length before sanitization for debugging/logging purposes. This type establishes the contract for the sanitization utility and enables consumers to log sanitization events without exposing implementation details.
*   `[✅]` 122. **`[TEST-UNIT]` `supabase/functions/_shared/utils/type-guards/type_guards.jsonSanitizer.test.ts` — RED tests for JSON sanitization type guards**
    *   `[✅]` 122.a. `[DEPS]` The `JsonSanitizationResult` type requires type guards to validate sanitized content and ensure type safety throughout the codebase. The type guard functions must prove that sanitized strings are valid JSON content and that result objects conform to the `JsonSanitizationResult` interface before consumers attempt parsing.
    *   `[✅]` 122.b. `[TYPES]` Before writing tests, identify all object shapes that will be created in the test file. Review existing types in `jsonSanitizer.interface.ts` and related type files. For each test object (mock `JsonSanitizationResult` objects, test fixture objects, etc.), either use the existing `JsonSanitizationResult` type or define a new well-typed shape in `jsonSanitizer.interface.ts` if needed. Ensure all test fixtures are built from concrete, exported types rather than inline object literals or `as` casts.
    *   `[✅]` 122.c. `[TEST-UNIT]` **RED**: In a new file `supabase/functions/_shared/utils/type-guards/type_guards.jsonSanitizer.test.ts`, write failing unit tests for the following type guards:
        *   `[✅]` 122.c.i. Test for `isJsonSanitizationResult`: Should return `true` for objects that match the `JsonSanitizationResult` interface (with `sanitized: string`, `wasSanitized: boolean`, `originalLength: number`) and `false` for objects missing required properties, having incorrect types, or being non-objects. Construct all test objects using the `JsonSanitizationResult` type from `jsonSanitizer.interface.ts` rather than inline object literals.
        *   `[✅]` 122.c.ii. Test for `isValidJsonString`: Should return `true` for strings that can be successfully parsed as JSON (valid JSON objects, arrays, primitives) and `false` for strings that are not valid JSON (malformed syntax, incomplete objects, non-JSON content). This test must fail because the type guard does not exist yet, proving the implementation gap.
    *   `[✅]` 122.d. `[LINT]` Run the linter for `supabase/functions/_shared/utils/type-guards/type_guards.jsonSanitizer.test.ts` and resolve any warnings or errors introduced by the new tests.
*   `[✅]` 123. **`[BE]` `supabase/functions/_shared/utils/type-guards/type_guards.jsonSanitizer.ts` — GREEN: Implement JSON sanitization type guards**
    *   `[✅]` 123.a. `[BE]` **GREEN**: In a new file `supabase/functions/_shared/utils/type-guards/type_guards.jsonSanitizer.ts`, implement and export the type guards:
        *   `[✅]` 123.a.i. Create and export `isJsonSanitizationResult`: This function validates that an unknown value conforms to the `JsonSanitizationResult` type by checking for the presence of `sanitized` (string), `wasSanitized` (boolean), and `originalLength` (number) properties with correct types.
        *   `[✅]` 123.a.ii. Create and export `isValidJsonString`: This function validates that a string can be parsed as valid JSON by attempting `JSON.parse()` and catching any errors. This provides type narrowing for sanitized content before consumers attempt parsing.
    *   `[✅]` 123.b. `[TEST-UNIT]` **GREEN**: Re-run the tests from step 122.c and ensure they now pass, proving the type guards correctly validate `JsonSanitizationResult` objects and JSON strings.
    *   `[✅]` 123.c. `[LINT]` Run the linter for `supabase/functions/_shared/utils/type-guards/type_guards.jsonSanitizer.ts` and resolve any warnings or errors.
*   `[✅]` 124. **`[TEST-UNIT]` `supabase/functions/_shared/utils/jsonSanitizer.test.ts` — RED tests for JSON sanitization utility function**
    *   `[✅]` 124.a. `[DEPS]` The JSON sanitization utility function must handle common AI response formatting deviations without altering valid JSON content. The function accepts a raw string (potentially wrapped in quotes, backticks, or markdown code blocks) and returns a `JsonSanitizationResult` containing the sanitized string, a flag indicating whether sanitization occurred, and the original length for debugging.
    *   `[✅]` 124.b. `[TYPES]` Before writing tests, identify all object shapes that will be created in the test file. Review existing types in `jsonSanitizer.interface.ts` and related type files (e.g., `type_guards.jsonSanitizer.ts`). For each test object (test fixture objects, expected result objects, etc.), either use the existing `JsonSanitizationResult` type or define a new well-typed shape in `jsonSanitizer.interface.ts` if needed. Ensure all test fixtures and expected result objects are built from concrete, exported types rather than inline object literals or `as` casts. Verify that `jsonSanitizer.interface.ts` contains complete type definitions for every shape used in both tests and source.
    *   `[✅]` 124.c. `[TEST-UNIT]` **RED**: In a new file `supabase/functions/_shared/utils/jsonSanitizer.test.ts`, write failing unit tests for the `sanitizeJsonContent` function. The tests must:
        *   `[✅]` 124.c.i. Test case: "should pass through valid JSON without sanitization". Provide a `rawContent` string containing valid JSON (e.g., `'{"key": "value"}'`) with no wrappers. Construct the expected result object using the `JsonSanitizationResult` type from `jsonSanitizer.interface.ts`. Assert the function returns a `JsonSanitizationResult` with `sanitized` equal to the trimmed input, `wasSanitized: false`, and `originalLength` matching the input length.
        *   `[✅]` 124.c.ii. Test case: "should remove surrounding single quotes". Provide a `rawContent` string like `'\'{"key": "value"}\''` (single quotes wrapping JSON). Construct the expected result object using the `JsonSanitizationResult` type. Assert the function returns `sanitized: '{"key": "value"}'`, `wasSanitized: true`, and validates that the sanitized string is valid JSON via `JSON.parse()`.
        *   `[✅]` 124.c.iii. Test case: "should remove surrounding double quotes when not JSON". Provide a `rawContent` string like `'"{"key": "value"}"'` (double quotes wrapping JSON object). Construct the expected result object using the `JsonSanitizationResult` type. Assert the function returns `sanitized: '{"key": "value"}'`, `wasSanitized: true`, and the sanitized string is valid JSON.
        *   `[✅]` 124.c.iv. Test case: "should preserve double quotes when JSON starts with object". Provide a `rawContent` string like `'{"key": "value"}'` (no wrapper, just valid JSON). Construct the expected result object using the `JsonSanitizationResult` type. Assert the function does not strip the double quotes inside the JSON and returns `wasSanitized: false`.
        *   `[✅]` 124.c.v. Test case: "should remove triple backticks with json language tag". Provide a `rawContent` string like `` '```json\n{"key": "value"}\n```' ``. Construct the expected result object using the `JsonSanitizationResult` type. Assert the function returns `sanitized: '{"key": "value"}'`, `wasSanitized: true`, and the sanitized string is valid JSON.
        *   `[✅]` 124.c.vi. Test case: "should remove triple backticks without language tag". Provide a `rawContent` string like `` '```\n{"key": "value"}\n```' ``. Construct the expected result object using the `JsonSanitizationResult` type. Assert the function returns `sanitized: '{"key": "value"}'`, `wasSanitized: true`.
        *   `[✅]` 124.c.vii. Test case: "should trim leading and trailing whitespace". Provide a `rawContent` string like `'  \n  {"key": "value"}  \n  '`. Construct the expected result object using the `JsonSanitizationResult` type. Assert the function returns `sanitized: '{"key": "value"}'`, `wasSanitized: true`, and trims all whitespace.
        *   `[✅]` 124.c.viii. Test case: "should handle nested wrapper patterns". Provide a `rawContent` string like `` '\'```json\n{"key": "value"}\n```\'' `` (single quotes wrapping backticks). Construct the expected result object using the `JsonSanitizationResult` type. Assert the function removes both wrapper layers and returns `sanitized: '{"key": "value"}'`, `wasSanitized: true`.
        *   `[✅]` 124.c.ix. Test case: "should preserve valid JSON array content". Provide a `rawContent` string like `'\'[{"key": "value"}]\''`. Construct the expected result object using the `JsonSanitizationResult` type. Assert the function returns `sanitized: '[{"key": "value"}]'`, `wasSanitized: true`, and the sanitized string parses to a valid array.
        *   `[✅]` 124.c.x. Test case: "should handle empty JSON objects and arrays". Provide `rawContent` strings like `'\'{}\''` and `'\'[]\''`. Construct the expected result objects using the `JsonSanitizationResult` type. Assert the function returns `sanitized: '{}'` and `sanitized: '[]'` respectively, both with `wasSanitized: true`.
        *   `[✅]` 124.c.xi. All tests must fail because the `sanitizeJsonContent` function does not exist yet, proving the implementation gap.
    *   `[✅]` 124.d. `[LINT]` Run the linter for `supabase/functions/_shared/utils/jsonSanitizer.test.ts` and resolve any warnings or errors introduced by the new tests.
*   `[✅]` 125. **`[BE]` `supabase/functions/_shared/utils/jsonSanitizer.ts` — GREEN: Implement JSON sanitization utility function**
    *   `[✅]` 125.a. `[TYPES]` Before implementing the function, verify that `jsonSanitizer.interface.ts` contains complete type definitions for every shape used in this source file. Review the function signature, internal variables, intermediate objects, and return type. Ensure all object shapes are either using existing types (e.g., `JsonSanitizationResult` from `jsonSanitizer.interface.ts`) or have been defined as new exported types in `jsonSanitizer.interface.ts` if needed. The completion of this step should produce a complete type file that defines every shape used in both tests and source.
    *   `[✅]` 125.b. `[BE]` **GREEN**: In a new file `supabase/functions/_shared/utils/jsonSanitizer.ts`, implement and export the `sanitizeJsonContent` function:
        *   `[✅]` 125.b.i. Import the `JsonSanitizationResult` type from `jsonSanitizer.interface.ts`.
        *   `[✅]` 125.b.ii. Implement `sanitizeJsonContent(rawContent: string): JsonSanitizationResult`: This function must (1) store the original length for the result, (2) trim leading and trailing whitespace, (3) remove triple backticks (with optional `json` or `JSON` language tag) from start and end if present, (4) remove surrounding single quotes if they match at start/end, (5) remove surrounding double quotes only if they wrap the entire content and the content does not start with `{` or `[` (to avoid stripping quotes from valid JSON), (6) return a `JsonSanitizationResult` with the sanitized string, a `wasSanitized` flag indicating if any operations were performed, and `originalLength`. Construct all internal objects using the `JsonSanitizationResult` type rather than inline object literals.
        *   `[✅]` 125.b.iii. Ensure the function handles nested wrapper patterns by applying removal operations in the correct order (backticks first, then quotes, then trimming).
        *   `[✅]` 125.b.iv. Ensure the function does not mutate valid JSON content that does not require sanitization (e.g., JSON that starts with `{` or `[` and has no wrapper characters).
    *   `[✅]` 125.c. `[TEST-UNIT]` **GREEN**: Re-run all tests from step 124.c and ensure they now pass, proving the `sanitizeJsonContent` function correctly handles all common wrapper patterns and preserves valid JSON content.
    *   `[✅]` 125.d. `[LINT]` Run the linter for `supabase/functions/_shared/utils/jsonSanitizer.ts` and resolve any warnings or errors.
*   `[✅]` 126. **`[TEST-UNIT]` `supabase/functions/dialectic-worker/executeModelCallAndSave.test.ts` — RED: Update tests to use JSON sanitization utility**
    *   `[✅]` 126.a. `[DEPS]` The `executeModelCallAndSave` function currently uses inline JSON parsing logic with immediate retry on failure (lines 933-957). The existing test at line 1694 verifies that malformed JSON triggers a retry. After integrating the JSON sanitization utility, the function should first attempt sanitization before retrying, meaning truly malformed JSON (that cannot be fixed by sanitization) should still trigger retry, but common wrapper patterns should be handled without retry.
    *   `[✅]` 126.b. `[TYPES]` Before updating tests, identify all object shapes that will be created or modified in the test file. Review existing types in `jsonSanitizer.interface.ts`, `dialectic.interface.ts`, and related type files. For each test object (mock AI response objects, mock `UnifiedAIResponse` objects, expected result objects, etc.), either use existing types (e.g., `JsonSanitizationResult`, `UnifiedAIResponse` from `dialectic.interface.ts`) or define new well-typed shapes in the appropriate type file if needed. Ensure all test fixtures and mock objects are built from concrete, exported types rather than inline object literals or `as` casts. Verify that all types used in tests are accessible and defined in the appropriate type files.
    *   `[✅]` 126.c. `[TEST-UNIT]` **RED**: In `supabase/functions/dialectic-worker/executeModelCallAndSave.jsonSanitizer.test.ts`, update the existing test `'when the model produces malformed JSON, it should trigger a retry, not a continuation'` (line 1694) to verify the new behavior:
        *   `[✅]` 126.c.i. Modify the test setup to provide a mock AI response with content `'\'{"key": "value", "incomplete\''` (single quotes wrapping incomplete JSON). Construct all mock objects using existing types from `dialectic.interface.ts` (e.g., `UnifiedAIResponse`) rather than inline object literals. This tests that common wrapper patterns are sanitized first, then the still-malformed content triggers retry.
        *   `[✅]` 126.c.ii. Add a new test case `'when the model produces JSON wrapped in common patterns, it should sanitize and parse successfully'` that provides a mock AI response with content `'\'{"key": "value"}\''` (single quotes wrapping valid JSON). Construct all mock objects using existing types. Assert that `executeModelCallAndSave` successfully parses the JSON after sanitization, does not call `retryJob`, and proceeds with normal processing.
        *   `[✅]` 126.c.iii. Add a new test case `'when the model produces JSON wrapped in triple backticks, it should sanitize and parse successfully'` that provides a mock AI response with content `` '```json\n{"key": "value"}\n```' ``. Construct all mock objects using existing types. Assert successful parsing without retry.
        *   `[✅]` 126.c.iv. Ensure the existing test for truly malformed JSON (incomplete syntax that cannot be fixed by sanitization) still asserts retry is triggered, proving that sanitization does not mask real parsing failures.
        *   `[✅]` 126.c.v. These tests must fail because `executeModelCallAndSave` does not yet use the sanitization utility, proving the implementation gap.
    *   `[✅]` 126.d. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/executeModelCallAndSave.jsonSanitizer.test.ts` and resolve any warnings or errors introduced by the test updates.
*   `[✅]` 127. **`[BE]` `supabase/functions/dialectic-worker/executeModelCallAndSave.ts` — GREEN: Replace inline JSON parsing with sanitization utility**
    *   `[✅]` 127.a. `[TYPES]` Before implementing the changes, identify all object shapes that will be created or modified in this source file. Review existing types in `jsonSanitizer.interface.ts`, `dialectic.interface.ts`, and related type files. For each object created in the implementation (variables holding `JsonSanitizationResult`, intermediate objects, function parameters, etc.), either use existing types (e.g., `JsonSanitizationResult` from `jsonSanitizer.interface.ts`, `UnifiedAIResponse` from `dialectic.interface.ts`) or define new well-typed shapes in the appropriate type file if needed. Verify that all object shapes used in this source file are defined and accessible in type files. The completion of this step should ensure that `jsonSanitizer.interface.ts` and related type files contain complete type definitions for every shape used in both tests and source, including the function's DI dependencies (already noted in `ExecuteModelCallAndSaveParams`) and return signature (already typed as `void` or the existing return type).
    *   `[✅]` 127.b. `[BE]` **GREEN**: In `supabase/functions/dialectic-worker/executeModelCallAndSave.ts`, replace the inline JSON parsing logic (lines 933-957) with the sanitization utility:
        *   `[✅]` 127.b.i. Import `sanitizeJsonContent` from `'../_shared/utils/jsonSanitizer.ts'` and import `isJsonSanitizationResult` from `'../_shared/utils/type-guards/type_guards.jsonSanitizer.ts'`.
        *   `[✅]` 127.b.ii. Before the existing `JSON.parse()` attempt, call `sanitizeJsonContent(aiResponse.content)` to obtain a `JsonSanitizationResult`. Store the result in a variable typed as `JsonSanitizationResult` using the type from `jsonSanitizer.interface.ts`.
        *   `[✅]` 127.b.iii. Use `isJsonSanitizationResult` to validate the result object for type safety before proceeding.
        *   `[✅]` 127.b.iv. Attempt `JSON.parse(result.sanitized)` using the sanitized content instead of the raw content.
        *   `[✅]` 127.b.v. If `result.wasSanitized` is `true`, log an informative message (e.g., `deps.logger.info`) indicating that sanitization was performed, including the original length and sanitized length for debugging purposes.
        *   `[✅]` 127.b.vi. Only trigger retry if parsing the sanitized content still fails, maintaining the existing retry logic for truly malformed JSON.
        *   `[✅]` 127.b.vii. Remove the old inline parsing logic and ensure all error handling paths remain intact.
    *   `[✅]` 127.c. `[TEST-UNIT]` **GREEN**: Re-run all tests from step 126.c and ensure they now pass, proving `executeModelCallAndSave` correctly uses the sanitization utility and handles both common wrapper patterns and truly malformed JSON appropriately.
    *   `[✅]` 127.d. `[TEST-UNIT]` Re-run the existing test suite in `executeModelCallAndSave.test.ts` to ensure no regressions were introduced by replacing the inline parsing logic.
    *   `[✅]` 127.e. `[TYPES]` After implementation, verify that `jsonSanitizer.interface.ts` contains complete type definitions for every shape used in both tests and source files. Review all test files and source files created or modified in steps 121-127 to ensure every object shape (mock objects, fixtures, function parameters, return values, intermediate variables, DI dependencies) is either using an existing type or defined as a new exported type in the appropriate type file. This final verification ensures the type file is complete and all shapes are accessible.
    *   `[✅]` 127.f. `[LINT]` Run the linter for `supabase/functions/dialectic-worker/executeModelCallAndSave.ts` and resolve any warnings or errors.
    *   `[✅]` 127.g. `[CRITERIA]` The implementation is complete when: (1) all tests in `executeModelCallAndSave.test.ts` pass (including new sanitization tests and existing malformed JSON tests), (2) `executeModelCallAndSave` successfully parses JSON wrapped in common patterns (single quotes, double quotes, triple backticks, markdown code blocks) without retry, (3) `executeModelCallAndSave` still triggers retry for truly malformed JSON that cannot be fixed by sanitization, (4) sanitization events are logged appropriately for debugging, (5) the inline JSON parsing/retry logic has been completely replaced with the reusable utility, (6) `jsonSanitizer.interface.ts` contains complete type definitions for every shape used in tests and source files (including all mock objects, fixtures, function parameters, return values, intermediate variables, and DI dependencies), (7) all object shapes in tests and source are either using existing types or defined as exported types in type files, (8) the linter reports no errors or warnings.
