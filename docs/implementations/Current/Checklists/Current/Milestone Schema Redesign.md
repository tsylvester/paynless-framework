...
*   `[ ]`   [DB] supabase/migrations/`{timestamp}_paralysis_checklist_nodes.sql` **Update paralysis recipe Step 2: replace content_to_include and assembled_json for Example Checklist structure**
    *   `[ ]`   `objective`
    ...
    *   `[ ]`   `{timestamp}_paralysis_checklist_nodes.sql`
...
## Milestone 3: Validation and Documentation Sync
