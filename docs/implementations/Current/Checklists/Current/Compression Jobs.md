...
* `[ ]`   supabase/functions/_shared/utils/`path_constructor.ts` **[BE] Add the canonical CompressedContext storage path (final and map-reduce-chunk forms), and own the new FileType member and CompressionSourceType union every downstream compression node imports**

  * `[ ]`   `objective`
    * `[ ]`   Solve the missing deterministic storage location for a COMPRESS job's persisted output. `constructStoragePath` (`path_constructor.ts:52-505`) has a `switch (fileType)` case for every existing artifact kind, but no case exists for a compressed-victim artifact; there is also no `FileType` member, no `PathContext` fields, and no `CompressionSourceType` union to describe one. Per `Compression Jobs Scope.md`'s TYPE OWNERSHIP block, this node is the OWNER of `FileType.CompressedContext` and `CompressionSourceType` — every later node (`enqueueCompressJobs`, `processCompressJob`, `saveResponse`, `vector_utils`, `compressPrompt`, `applyCompressionOverlay`) imports these from here rather than redeclaring them.
    * `[ ]`   Functional goals:
  ...
  * `[ ]`   `role`
    * `[ ]`   Node role: canonical path-construction implementation and the OWNING node for `FileType.CompressedContext` and `CompressionSourceType`, per `Compression Jobs Scope.md`'s TYPE OWNERSHIP block. This role is correct because `path_constructor.ts` is where every other artifact's canonical path already lives, and per the module-first rule, a type belongs in the interface/types file of the module that owns it — path/file identity is exactly what `file_manager.types.ts` (this module's types file) already exists to describe.
    * `[ ]`   Out-of-scope responsibilities (each its own later node, per `Compression Jobs Scope.md`'s Sprint 2/3 node list):
      ...
      * `[ ]`   Defining `CompressionMode` or `DialecticCompressJobPayload` (owned by `enqueueCompressJobs.interface.ts`, Sprint 3, per TYPE OWNERSHIP).
  ...
  * `[ ]`   `module`
    ...
    * `[ ]`   Inside boundary: the `FileType.CompressedContext` enum member, the `CompressionSourceType` union, the five new `PathContext` fields, the new switch case's directory/filename derivation, the two new type guards.
  ...
  * `[ ]`   `deps`
    ...
    * `[ ]`   Confirm: no reverse dependency (nothing existing references `FileType.CompressedContext` or `CompressionSourceType`); no lateral violation (this node does not import from `dialectic-worker/` or any other module).

  * `[ ]`   `context_slice`
    * `[ ]`   From `PathContext`: the new case reads exactly `projectId`, `sessionId`, `iteration`, `stageSlug` (via the existing `stageRootPath` computation, unchanged), `targetKey`, `sourceType`, `documentKey`, `sourceId`, `chunkIndex`, `chunkTotal` — no other existing `PathContext` field (e.g. `modelSlug`, `attemptCount`, `contributionType`) is read by this case, matching the `RagContextSummary` case's precedent of using only the subset of context fields it actually needs.
  ...
  * `[ ]`   `constructStoragePath.interaction.spec` (prose; no file — matches the repo's existing precedent of no literal `.interaction.spec` file for this function)
    * `[ ]`   Called by (this sprint and later): `file_manager.ts` (this sprint, next-but-one node) when persisting a `CompressedContext` resource; `path_deconstructor.ts` is the read-side counterpart (parses the paths this case produces, not a caller of this function).
  ...
  * `[ ]`   `type_guards.file_manager.ts` (real file playing the Enforcement/guard.ts role)
    ...
    * `[ ]`   Import `CompressionSourceType` alongside the existing `FileType`/`CanonicalPathParams`/etc. import block (`type_guards.file_manager.ts:2-13`) from `'../../types/file_manager.types.ts'`.
  ...
  * `[ ]`   `construction`
    ...
    * `[ ]`   Initialization order inside the new case: validate `stageRootPath`/`targetKey`/`sourceType` first (unconditional), then branch on `sourceType` to validate its ONE required identity field (`documentKey` or `sourceId`), then validate chunk-field pairing and chunk-number bounds, THEN compute `source_basename` from the already-validated branch, THEN compute the final filename — no computation happens before its preconditions are confirmed, matching the file's existing top-to-bottom validate-then-compute pattern.
  ...
  * `[ ]`   `path_constructor.ts` (Implementation)
    ...
    * `[ ]`   Each requirement above maps 1:1 to a code path exercised by `path_constructor.test.ts`.
  ...
  * `[ ]`   `file_manager.types.ts` (companion type-owner edit riding this node, per TYPE OWNERSHIP)
    ...
    * `[ ]`   Add the five new optional fields (`targetKey?: string; sourceType?: CompressionSourceType; sourceId?: string; chunkIndex?: number; chunkTotal?: number;`) to the `PathContext` interface (`file_manager.types.ts:110-135`), each with a one-line comment naming its purpose, matching this interface's existing per-field comment style.

  * `[ ]`   `path_constructor.provides.ts` — NONE. No `.provides.ts` file exists for this utility; consumers already import `constructStoragePath`, `ConstructedPath`, `sanitizeForPath`, and `generateShortId` directly from `path_constructor.ts`, and `PathContext`/`FileType`/`CompressionSourceType` directly from `file_manager.types.ts`. This node changes none of those import paths.
  ...
  * `[ ]`   `requirements` (binary, observable)
    ...
    * `[ ]`   `isCompressedContextFileType` and `isCompressionSourceType` correctly accept/reject per their guard tests; `isResourceFileType(FileType.CompressedContext)` returns `true`.
...
* `[ ]`   supabase/functions/_shared/prompt-assembler/`assembleCompressionPrompt.ts` **[BE] Assemble the mode-aware compression prompt for a COMPRESS job by loading the seeded template and rendering it against the victim content and the consuming step's target schema**
  ...
  * `[ ]`   `assembleCompressionPrompt.integration.test.ts`
    * `[ ]`   Bounded subsystem: real implementation, real `renderPrompt`, a mock Supabase client seeded with the ACTUAL Sprint-1 migration template text (copy the exact `prompt_text` body, not a paraphrase) standing in for the Postgres boundary.
    * `[ ]`   Assert the rendered `prompt` contains fixed template prose that is NOT a substituted variable (e.g. "Condense string values in place." appears only in `json` mode output), proving the real seeded template is what's being rendered.
    * `[ ]`   Call the function exactly as `processCompressJob.ts` will: full params/payload via the Trusted Factories, asserting the returned `prompt` is ready to hand directly to a model call (non-empty, no unsubstituted `{{...}}` tokens for any of the five known variables, no leftover `{{#section:...}}` tags).
    * `[ ]`   Both modes, both with and without chunk context — four scenarios total.
...
* `[ ]`   supabase/functions/dialectic-worker/enqueueModelCall/`enqueueModelCall.ts` **[BE] Accept FileType.CompressedContext as a valid output_type alongside model-contribution types, so a COMPRESS job's stream call is not rejected by the enqueue guard**
  ...
  * `[ ]`   `enqueueModelCall.integration.test.ts`
    * `[ ]`   New bounded-subsystem case, modeled on this file's existing `FileType`-based happy-path fixture (`enqueueModelCall.integration.test.ts:1-40`): a full real call with `output_type: FileType.CompressedContext`, real `computeJobSig` mock, real Supabase/fetch mocks, asserts `{ queued: true }` and that the posted event body's `chat_api_request` matches the payload — proving the widened guard doesn't just avoid throwing but lets a real COMPRESS-shaped call complete the whole function body.
...
* `[ ]`   supabase/functions/dialectic-worker/`processCompressJob.ts` **[BE] Process a COMPRESS job: validate its payload, skip if already compressed (dedup layer 2), assemble the compression prompt, enforce the recursion guard, and enqueue the stream call**
  ...
  * `[ ]`   `objective`
    ...
    * `[ ]`   Functional goals:
      * `[ ]`   Dedup layer 2: recompute the victim's canonical FINAL-artifact path via `deps.constructStoragePath` (the `FileType.CompressedContext` case) from `params`/`payload` fields (`projectId`, `sessionId`, `iterationNumber`, `stageSlug`, `targetKey`, `sourceType`, `documentKey`/`sourceId` — the same fields `enqueueCompressJobs`'s own dedup-layer-1 check uses) and query `dialectic_project_resources` by `(storage_path, file_name)` via `params.dbClient`. If found: mark the job row `status:'completed'` and return `{ queued: false }` — no model call spent, mirroring `enqueueCompressJobs`'s own "success, not error" framing for its dedup layer.
      * `[ ]`   Look up the `ai_providers` row for `payload.model_id` and validate its `config` via `isAiModelExtendedConfig` — identical validation to `enqueueModelCall.ts:29-36` — for use both in the recursion-guard sizing check below and as `EnqueueModelCallParams.providerRow`. Query failure or invalid config → `{ error, retriable }` (query failure retriable; invalid config not).
      * `[ ]`   Call `deps.assembleCompressionPrompt(assembleParams, assemblePayload)` — the pre-bound facade closure (see `deps` below) — with `assemblePayload: { mode: payload.mode, content: payload.content, chunk_index: payload.chunk_index, chunk_total: payload.chunk_total }` and `assembleParams.consumingStep` resolved from the CONSUMING stage's recipe step row (queried via `params.dbClient` by `payload.stageSlug`/`targetKey` — the COMPRESS job's own row has no recipe step of its own, per `assembleCompressionPrompt.ts`'s own objective). A returned error becomes this function's own `{ error, retriable }` — never retried into another compression.
      * `[ ]`   Recursion guard: size the ASSEMBLED prompt — `deps.countTokens(tokenizerDeps, { message: prompt }, providerRow.config)` — against `providerRow.config.provider_max_input_tokens` minus the same 32-token safety buffer established in `compressPrompt.ts`/`enqueueCompressJobs.ts`. `tokenizerDeps` is constructed inline exactly like `calculateAffordability.ts`'s own established placeholder (`calculateAffordability.ts:33-39`) — not a dep, since no existing caller threads it as one. If the assembled prompt STILL exceeds the budget: `{ error: new CompressJobEnqueueError(...), retriable: false }` — this IS the recursion guard (design decision 1: "a COMPRESS call that cannot fit its model window is a hard failure, not a nested compression"). Nearly unreachable per the ratified design; a defensive assertion, not an expected branch.
      * `[ ]`   Build `ChatApiRequest { message: prompt, providerId: payload.model_id, promptId: '__none__', max_tokens_to_generate: providerRow.config.provider_max_output_tokens }` (`message`/`providerId`/`promptId` are the only REQUIRED fields, `types.ts:204-207`; `promptId: '__none__'` is the documented literal for "no system_prompts template applies" — the compression prompt is already a fully-assembled instruction string) and call `deps.enqueueModelCall(enqueueParams, enqueuePayload)` with `output_type: FileType.CompressedContext`, `providerRow`, `userConfig: { tier_output_cap_tokens: null }` (matching the exact fixture shape at `enqueueModelCall.test.ts:149` — COMPRESS is infrastructure, not subject to user tier output caps). Its return propagates: success → `{ queued: true }`; error → this function's own `{ error, retriable }`.
    * `[ ]`   Non-functional constraints:
  ...
  * `[ ]`   `processCompressJob.interface.ts`
    ...
    * `[ ]`   Define `ProcessCompressJobParams { dbClient: SupabaseClient<Database>; jobId: string; projectOwnerUserId: string; authToken: string }` — `jobId` (not the full job row) is sufficient; this function reads what it needs from `payload` and looks up the provider/recipe-step rows itself via `dbClient`.
    * `[ ]`   Define `ProcessCompressJobPayload = DialecticCompressJobPayload` (imported from the `enqueueCompressJobs` module — not redefined).
    * `[ ]`   Define `ProcessCompressJobSuccessReturn { queued: boolean }` (`false` = dedup hit, nothing spent; `true` = model call enqueued), `ProcessCompressJobErrorReturn { error: Error; retriable: boolean }`, `ProcessCompressJobReturn = SuccessReturn | ErrorReturn`.
  ...
  * `[ ]`   `processCompressJob.interaction.spec` (prose; no file, matching this directory's established precedent)
    ...
    * `[ ]`   Required interactions: one dedup-existence read; on a miss, one `ai_providers` read, one `assembleCompressionPrompt` call, one `enqueueModelCall` call; on a hit, one `dialectic_generation_jobs` status write and nothing else.
    * `[ ]`   Failure modes: dedup-query failure (retriable), provider-lookup/invalid-config (query failure retriable, invalid config not), assembly failure (as returned), recursion-guard budget failure (not retriable), enqueue failure (as returned).
  ...
  * `[ ]`   `processCompressJob.mock.ts`
    * `[ ]`   `createProcessCompressJobMock(options?: { result?; handler? })` returning `{ processCompressJob, calls }`, structured exactly like `createEnqueueRenderJobMock`/`createCreateCompressJobsMock`; default fallback `{ queued: false }`.
    * `[ ]`   Trusted Factories: `buildProcessCompressJobDeps(overrides?)` (default `assembleCompressionPrompt`/`enqueueModelCall` from their own mock factories, `constructStoragePath` real, `countTokens` a simple deterministic stub), `buildProcessCompressJobParams(overrides?)`, `buildDialecticCompressJobPayload(overrides?)` (reuse/extend the one already defined in `enqueueCompressJobs.mock.ts` rather than duplicating).

  * `[ ]`   `processCompressJob.test.ts`
    * `[ ]`   Dedup hit → job row `status:'completed'`; `{ queued: false }`; `assembleCompressionPrompt`/`enqueueModelCall` never called.
    * `[ ]`   `ai_providers` lookup failure / invalid config → `{ error, retriable }` per case; no assembler/enqueue call.
    ...
    * `[ ]`   Assembled prompt exceeds the budget (recursion guard) → `{ error, retriable: false }`; `enqueueModelCall` never called; no new job row of any `job_type` is inserted (proves no nested compression).
    * `[ ]`   Happy path: dedup miss, valid provider, successful assembly, prompt within budget → `enqueueModelCall` called with `output_type: FileType.CompressedContext` and `chatApiRequest.message` equal to the assembled prompt; `{ queued: true }`; job status is NOT independently set to `'completed'` by this function.
    * `[ ]`   No notification-service calls on any path.

  * `[ ]`   `construction`
    * `[ ]`   No factory beyond the exported function. Dedup check → provider lookup/validation → assemble prompt → recursion-guard size check → build `ChatApiRequest` → `enqueueModelCall` — no DB write happens before the dedup check resolves.
  ...
  * `[ ]`   `processCompressJob.integration.test.ts`
    * `[ ]`   Bounded subsystem: real `processCompressJob`, real `assembleCompressionPrompt` (via a real `PromptAssembler` instance), real `constructStoragePath`, real `countTokens`; only Supabase and the Netlify POST (inside `enqueueModelCall`) are mocked.
    * `[ ]`   A dedup-miss victim produces a real assembled prompt containing the victim content and target schema, and a real `enqueueModelCall` invocation carrying that exact prompt.
  ...
  * `[ ]`   `requirements`
    ...
    * `[ ]`   A successful run calls `enqueueModelCall` with `output_type: FileType.CompressedContext` and returns `{ queued: true }`; this function does not independently mark the job `completed`.
...
* `[ ]`   supabase/functions/dialectic-worker/`processJob.ts` **[BE] Route COMPRESS jobs to processCompressJob via a new switch case, narrowing the payload with isDialecticCompressJobPayload and writing the job's own failed status when processCompressJob returns an error — the legacy hub's single touch for JobType/JobTypes, the DialecticJobPayload union, and IJobProcessors**
  ...
  * `[ ]`   `objective`
    ...
    * `[ ]`   Functional goals:
      ...
      * `[ ]`   Construct `ProcessCompressJobParams { dbClient, jobId: job.id, projectOwnerUserId, authToken }` — all four values are already parameters of `processJob` itself; no new lookup.
