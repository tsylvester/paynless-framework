...
| A node element that specifies a type surface by member count (`asserts twenty-nine`), or by a delta against its prior state (`drops ten, keeps every other member`) | The element that **declares** the surface names every member and its disposition. A count and a delta both leave the reader to rebuild the list the element declined to write, and both survive being wrong |
| The same surface enumerated again in a second element — the interface test, the mock, the guard and the requirements each restating the member list | One enumeration, in the declaring element; every other element **references** that element and states only what its own standard cannot derive. Copies of one surface drift apart, and each copy is another chance to be wrong |
