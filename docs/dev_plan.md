...
4.  **Error Handling Strategy:** Employ a layered approach to error handling for a robust user experience:
    *   **Global/Critical Errors:** Handle critical application-level errors (e.g., failure to load essential configuration or platform capabilities) gracefully. This might involve displaying a global error banner or a simplified error page, potentially using state provided by core services like `PlatformProvider` (`capabilityError`). Log these errors using the `logger` utility (`@paynless/utils`).
    *   **UI Rendering Errors:** Strategically wrap major UI sections (e.g., page routes, complex widgets) with React Error Boundaries (`@/components/common/ErrorBoundary.tsx`) to catch rendering exceptions within specific component trees. These boundaries should prevent cascading failures and display user-friendly fallback UIs or messages.
    *   **Asynchronous & Logic Errors (Store/Component Level):** Explicitly handle errors originating from API interactions, state management operations, or other business logic within the relevant store actions or component handlers (following the "Action Handling Pattern" described in the State Management section). Provide clear, user-friendly feedback through non-intrusive mechanisms like toast notifications (`sonner`) or inline error messages within forms (`Alert` component) and components (`StatusDisplay` pattern). Avoid disrupting the user's flow unnecessarily for recoverable errors. Log relevant details using the `logger`.
