...
async function handleStreamingRequest(params: {
	...
	logger.info("[handleStreamingRequest] Starting streaming request...");
	...
	try {
		...
		const streamingUrl = `${supabaseUrl}/functions/v1/chat-stream`;
				...
				for (const line of lines) {
					if (line.startsWith("event: ") && line.includes("chunk")) {
						...
						if (nextLine?.startsWith("data: ")) {
							try {
								...
								if (data.content) {
									...
									streamingCallbacks.onChunk?.(data.content);
								}
							} catch (e) {
								logger.warn(
									"[handleStreamingRequest] Failed to parse chunk data:",
									{ error: e },
								);
							}
						}
						...
						if (nextLine?.startsWith("data: ")) {
							try {
								const data = JSON.parse(nextLine.substring(6));
								const result: InternalProcessResult = {
									success: true,
									finalUserMessage: data.userMessage,
									assistantMessage: {
										...data.assistantMessage,
										content: fullContent || data.assistantMessage?.content,
									},
									newlyCreatedChatId: data.chatId,
									actualCostWalletTokens: data.tokenUsage?.cost,
									wasRewind: false,
								};
								...
								logger.warn(
									"[handleStreamingRequest] Failed to parse complete data:",
									{ error: parseError },
								);
							}
						}
		...
		// If we get here without a complete event, create a result with the accumulated content
		return {
			success: true,
			assistantMessage: {
				id: `temp_${Date.now()}`,
				...
				user_id: "",
				ai_provider_id: targetProviderId,
				...
				system_prompt_id: targetPromptId,
			} as ChatMessage,
			newlyCreatedChatId: targetChatId || undefined,
			wasRewind: false,
		};
	} catch (error: unknown) {
		const err = error instanceof Error ? error : new Error(String(error));
		logger.error("[handleStreamingRequest] Exception:", { err });
		streamingCallbacks.onError?.(err.message);
		return {
			success: false,
			error: err.message,
...
async function coreMessageProcessing(params: {
	...
	logger.info("[coreMessageProcessing] Starting...", {
		targetProviderId,
		targetChatId,
		continueUntilComplete,
		enableStreaming,
	...
	try {
		// If streaming is enabled, use the chat-stream endpoint
		if (enableStreaming && streamingCallbacks) {
...
export async function handleSendMessage(
	...
	const {
		message,
		chatId: inputChatId,
		contextMessages: providedContextMessages,
	} = data;
	const {
	...
	switch (activeWalletInfo.status) {
		...
		case "loading":
			aiStateService.setAiState({
				...
				aiError: currentUser
					? activeWalletInfo.message || "Wallet loading."
					: "Auth required.",
				pendingAction: currentUser ? undefined : "SEND_MESSAGE",
			});
		...
		case "error":
			aiStateService.setAiState({
				isLoadingAiResponse: false,
				aiError: `Wallet Error: ${activeWalletInfo.message || "Unknown"}`,
			});
			return null;
		case "consent_required":
			aiStateService.setAiState({
				...
				aiError:
					...
					"Please accept or decline the use of personal tokens for this organization chat.",
			});
			return null;
		case "consent_refused":
			aiStateService.setAiState({
				...
				aiError:
					activeWalletInfo.message ||
					"Chat disabled. Consent to use personal tokens was refused.",
			});
			return null;
		case "policy_org_wallet_unavailable":
			aiStateService.setAiState({
				...
				aiError:
					activeWalletInfo.message ||
					"Organization wallet is selected by policy, but not yet available.",
			});
			return null;
		default:
			aiStateService.setAiState({
				isLoadingAiResponse: false,
				aiError: activeWalletInfo.message || "Wallet issue.",
			});
			return null;
	}
	...
	const processingResult = await coreMessageProcessing({
		...
		targetPromptId: apiPromptId,
		targetChatId: effectiveChatIdForApi,
		selectedContextMessages: finalContextMessages,
		effectiveOrganizationId: organizationIdForApi,
		token: token,
		rewindTargetMessageId: currentRewindTargetId,
		continueUntilComplete: continueUntilComplete,
		enableStreaming: options?.enableStreaming,
		streamingCallbacks: options?.streamingCallbacks,
		callChatApi,
		logger,
	});
	...
	if (!processingResult.success || !processingResult.assistantMessage) {
		if (
			processingResult.errorCode === "AUTH_REQUIRED" ||
			processingResult.errorCode === "TOKEN_EXPIRED"
		) {
			aiStateService.setAiState((state) => {
				...
				newMsgsByChatId[optimisticMessageChatId] = chatMsgs.filter(
					(msg) => msg.id !== tempUserMessageId,
				);
				return {
					aiError: processingResult.error || "Authentication required.",
					isLoadingAiResponse: false,
					pendingAction: "SEND_MESSAGE",
			...
			try {
				const pendingActionDetails: PendingAction<ChatApiRequest> = {
					...
					body: {
						message,
						chatId: effectiveChatIdForApi ?? null,
						organizationId: newChatContext || null,
				...
				localStorage.setItem(
					...
					JSON.stringify(pendingActionDetails),
				);
			} catch (storageError) {
				logger.error("Failed to store pendingActionDetails:", { storageError });
			}
			authService.requestLoginNavigation();
		} else {
			aiStateService.setAiState((state) => {
				...
				return {
					aiError: processingResult.error || "Failed to send message.",
					isLoadingAiResponse: false,
					messagesByChatId: newMsgsByChatId,
				};
			});
	...
	aiStateService.setAiState((state: AiState) => {
		const isActualRewind =
			wasRewind ?? !!(inputChatId && currentRewindTargetId);
		...
		if (finalUserMessage) {
			messagesForChatProcessing = messagesForChatProcessing.map((msg) =>
				msg.id === tempUserMessageId
					? ({ ...finalUserMessage, status: "sent" as const } as ChatMessage)
					: msg,
			);
		} else {
			messagesForChatProcessing = messagesForChatProcessing.map((msg) =>
		...
		if (
			optimisticMessageChatId !== actualNewChatId &&
			newSelectedMessagesMap[optimisticMessageChatId]
		) {
		...
		if (isActualRewind) {
			...
			const selectionsForActualChat = {
				...(newSelectedMessagesMap[actualNewChatId] || {}),
			};
			...
			if (!finalUserMessageIdToSelect) {
				const processedUserMessage = messagesForChatProcessing.find(
					(msg: ChatMessage) =>
						msg.role === "user" && msg.id === tempUserMessageId,
				);
		...
		if (optimisticMessageChatId !== actualNewChatId) {
			...
			if (
				organizationIdForApi &&
				typeof organizationIdForApi === "string" &&
				organizationIdForApi !== "personal"
			) {
				const currentOrgChats =
					updatedChatsByContext.orgs?.[organizationIdForApi] || [];
		...
		const shouldClearNewChatContext =
			optimisticMessageChatId !== actualNewChatId;
		return {
			...state,
			...
			isLoadingAiResponse: false,
			aiError: null,
			rewindTargetMessageId: isActualRewind
				? null
				: state.rewindTargetMessageId,
			selectedMessagesMap: newSelectedMessagesMap,
			newChatContext: shouldClearNewChatContext ? null : state.newChatContext,
		};
	});

	logger.info("Message processed via handleSendMessage:", {
		assistantMessageId: assistantMessage.id,
		chatId: finalChatIdForLog,
		rewound: wasRewind,
	});
