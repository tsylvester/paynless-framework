...
describe('aiStore - deleteChat action', () => {
    ...
    it('should call startNewChat(null) if the deleted chat was the currentChatId', async () => {
        ...
        expect(finalState.currentChatId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/); // Should be a UUID generated by startNewChat
        ...
        // The currentChatId should be a new UUID from the new chat started by startNewChat
        expect(finalState.currentChatId).toBeDefined();
        expect(typeof finalState.currentChatId).toBe('string');
