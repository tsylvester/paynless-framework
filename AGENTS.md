# Agent Instructions

The single source of truth for how agents work in this repo is
[docs/agents/index.md](docs/agents/index.md).

Read that index and the topic files it points to. Every turn, follow the work loop
([docs/agents/loop.md](docs/agents/loop.md)) and obey the precedence order
([docs/agents/precedence.md](docs/agents/precedence.md)).

Everything an agent needs — how to work a turn, how to build each file, how to author
and implement a workplan node — is defined in the topics, once. Do not restate or
duplicate those rules here.
